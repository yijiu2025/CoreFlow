...
<template>
  <div class="absolute inset-0 z-10 pointer-events-none p-4 pt-20">
    <div class="max-w-6xl mx-auto pointer-events-auto">
      <div
        class="glass rounded-3xl p-6 shadow-2xl"
          ...
          >
            ...
            <div class="flex items-center justify-between mb-3">
              <div class="flex items-center gap-2">
                <span
                  class="text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400"
                  >L1</span
                >
                <span class="font-bold" :class="isDarkMode ? 'text-slate-200' : 'text-slate-800'">{{ sysKey }}</span>
              </div>
              <button
                @click="configsStore.toggleSystem(String(sysKey))"
            ...
            <div v-if="system.groups" class="ml-4 space-y-3">
              <div
                v-for="(group, grpKey) in system.groups"
                :key="grpKey"
              ...
              >
                <div class="flex items-center justify-between mb-2">
                  <div class="flex items-center gap-2">
                    <span
                      class="text-xs font-bold uppercase tracking-widest px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-400"
                      >L2</span
                    >
                ...
                <div v-if="group.apis" class="ml-6 space-y-1">
                  ...
                  >
                    <div class="flex items-center gap-2">
                      <span class="text-[10px] font-bold px-1.5 py-0.5 rounded bg-violet-500/20 text-violet-400"
                        >L3</span
                      >
                      <span class="font-mono" :class="isDarkMode ? 'text-slate-400' : 'text-slate-600'">{{
                        apiKey
                      }}</span>
                    </div>
                    <div class="flex gap-2">
                      <button
                        @click="configsStore.openEditModal(String(sysKey), String(grpKey), String(apiKey))"
                        class="text-[10px] px-2 py-0.5 rounded bg-indigo-500/20 text-indigo-400 hover:bg-indigo-500/30"
                      >
                        {{ $t('config.edit_endpoint') }}
                      </button>
                      <button
                        @click="configsStore.toggleGuard(String(grpKey), String(apiKey))"
