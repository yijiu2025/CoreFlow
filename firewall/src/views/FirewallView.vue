    ...
    <div class="max-w-6xl mx-auto pointer-events-auto space-y-6">
      ...
      >
        ...
        <div v-if="activeBlocks.length" class="space-y-2">
          <div
            ...
            :key="block.ip || block.fingerprint"
          ...
          >
            <div class="flex items-center gap-3">
              ...
              <span
                ...
                :class="
                  block.type === 'fingerprint' ? 'bg-purple-500/20 text-purple-400' : 'bg-blue-500/20 text-blue-400'
                "
              >
            ...
            <button
              @click="
                defenseStore.handleRemoveBlock({ type: block.type || 'ip', value: block.ip || block.fingerprint })
              "
      ...
      >
        ...
        <div v-if="activeWhitelist.length" class="space-y-2">
          <div
            v-for="wl in activeWhitelist"
            :key="wl.ip || wl.fingerprint"
            class="flex items-center justify-between p-3 rounded-xl"
            :class="isDarkMode ? 'bg-white/5' : 'bg-slate-50'"
          >
            <div class="flex items-center gap-3">
            ...
            <button
              @click="defenseStore.handleRemoveWhitelist({ type: wl.type || 'ip', value: wl.ip || wl.fingerprint })"
              class="text-xs text-red-400 hover:text-red-300 transition-colors"
            >
              {{ $t('common.remove') }}
            </button>
