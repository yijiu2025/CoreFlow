    ...
    <div class="absolute inset-0 z-10 pointer-events-none flex flex-col">
      ...
      <div class="fixed bottom-6 right-6 pointer-events-auto z-[70]">
        <button
          ...
          class="glass p-4 rounded-full transition-all group shadow-2xl"
          :class="
            isDarkMode
              ? 'hover:bg-cyan-500/20 text-cyan-400 border-cyan-500/30'
              : 'hover:bg-indigo-500/10 text-indigo-600 border-indigo-200'
          "
...
// 按需加载：打开安全控制台时加载配置
watch(
  () => defenseStore.isConfigModalOpen,
  val => {
...
watch(
  () => defenseStore.isSettingsModalOpen,
  val => {
...
watch(
  () => defenseStore.isDefenseModalOpen,
  val => {
    if (val) fetchSettingsOnDemand();
  }
);
...
async function fetchSettingsOnDemand(): Promise<void> {
  if (!settingsStore.loaded) {
    await Promise.all([settingsStore.fetchSettings(), defenseStore.fetchBlocks(), defenseStore.fetchWhitelist()]);
  }
}
