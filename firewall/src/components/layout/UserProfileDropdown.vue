<template>
  <div class="relative">
    ...
    >
      ...
      />
      <div
        v-else-if="authStore.isLoggedIn && authStore.user?.username"
      ...
      <div
        ...
        class="absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 bg-emerald-400"
    ...
    <transition name="dropdown-fade">
      ...
      >
        ...
        >
          <div
            class="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(255,255,255,0.15),transparent)]"
          ></div>
          <!-- Close -->
          <button
            @click="emit('close')"
            ...
            :class="
              isDarkMode ? 'bg-white/10 hover:bg-white/20 text-white/70' : 'bg-black/10 hover:bg-black/20 text-black/50'
            "
          >
            <X class="w-3.5 h-3.5" />
          </button>
        ...
        <div class="flex flex-col items-center -mt-10 relative z-10">
          <div class="relative">
            <div
              class="w-[76px] h-[76px] rounded-[22px] p-[3px] shadow-lg"
              :class="
                isDarkMode
            ...
            >
              <div
                class="w-full h-full rounded-[19px] overflow-hidden flex items-center justify-center"
        ...
        <div class="px-6 pt-3 pb-4 text-center">
          ...
          <!-- Role Badge -->
          <div
            class="mt-3 inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider"
            :class="
              authStore.isAdmin
                ? isDarkMode
            ...
            "
          >
            <span class="w-1.5 h-1.5 rounded-full" :class="authStore.isAdmin ? 'bg-rose-400' : 'bg-cyan-400'"></span>
            {{ authStore.roleName }}
          </div>
        </div>
        ...
        <div class="p-4 space-y-1">
          ...
          >
            ...
            <span
              class="text-[9px] px-1.5 py-0.5 rounded-md font-bold"
              :class="isDarkMode ? 'bg-violet-500/10 text-violet-400' : 'bg-violet-50 text-violet-600'"
              >PRO</span
            >
          </button>
...
import { User, X, LogOut, ShieldCheck, Settings, ChevronRight, ExternalLink } from 'lucide-vue-next';
