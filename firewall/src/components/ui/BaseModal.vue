<template>
  <div v-if="visible" class="fixed inset-0 h-screen flex items-center justify-center p-6" :class="zIndex">
    <!-- Backdrop: 毛玻璃效果 -->
    <div
      class="absolute inset-0 transition-opacity ease-out"
      :class="[backdropClass, backdropDuration, entered ? 'opacity-100' : 'opacity-0']"
      @click="closable && $emit('update:modelValue', false)"
    ></div>
    ...
    >
      <div
        v-if="entered"
        class="w-full flex flex-col overflow-hidden relative z-10 border transition-colors duration-500"
        :class="[
          sizeClass,
      ...
      >
        <!-- Header -->
        <div
          class="px-8 py-4 border-b flex items-center justify-between transition-colors duration-500"
          ...
          <div class="flex items-center gap-4">
            <div v-if="showDots" class="flex gap-1.5 mr-4">
              <div
                ...
                class="w-3 h-3 rounded-full bg-red-400 hover:bg-red-500 cursor-pointer transition-colors shadow-inner"
              ></div>
        ...
        <div
          v-if="$slots.footer"
          class="px-8 py-4 border-t flex items-center justify-between transition-colors duration-500"
