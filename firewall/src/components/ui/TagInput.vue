  ...
  <div>
    <div v-if="label" class="flex items-center justify-between mb-2">
      <label
        ...
        >{{ label }}</label
      >
      <span v-if="badge" class="text-[9px] font-bold px-2 py-0.5 rounded-md" :class="badgeClass">{{ badge }}</span>
    </div>
    <div
      class="flex flex-wrap gap-1.5 p-3 rounded-xl border min-h-[40px]"
    ...
    >
      ...
      >
        ...
        <X class="w-3 h-3 cursor-pointer opacity-60 hover:opacity-100" @click="$emit('remove', i)" />
      </span>
      <input
