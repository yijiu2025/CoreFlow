  ...
  >
    ...
    <div class="w-64 border-r border-var-border flex flex-col bg-var-side-bg backdrop-blur-md">
      <div class="p-6">
        ...
        <nav class="space-y-1">
          <div
            v-for="(system, sysKey) in configs"
            :key="sysKey"
            @click="selectSystem(sysKey)"
      ...
      <div v-if="currentSystemData" class="flex-1 flex flex-col min-h-0">
        ...
        <header class="px-10 py-8 border-b border-var-border bg-var-header-bg sticky top-0 z-10">
          <div class="flex items-start justify-between">
            <div>
              <div
                class="flex items-center gap-2 mb-2 text-[10px] font-bold uppercase tracking-widest"
              ...
              <div class="flex items-center gap-3 mb-2">
                <h1 class="text-3xl font-bold tracking-tight" :class="isDark ? 'text-white' : 'text-slate-900'">
                  {{ currentSystemData.alias || currentSystemData.name }}
                </h1>
                <span
        ...
        <div class="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar">
          ...
          <section>
            ...
            <div class="grid grid-cols-1 gap-3">
              <div
                v-for="(group, grpKey) in currentSystemData.groups"
                :key="grpKey"
              ...
              >
                <div class="flex items-center p-5 gap-6">
                    ...
                    <div
                      :class="
                        group.enabled
                          ...
                          : 'bg-slate-100 dark:bg-slate-900 text-slate-400 dark:text-slate-700'
                      "
                  ...
                  <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-3 mb-1">
                      <h4 class="text-base font-bold truncate" :class="isDark ? 'text-slate-200' : 'text-slate-900'">
                        {{ group.alias || group.name || grpKey }}
                      </h4>
                      <span
                        class="px-1.5 py-0.5 rounded text-[9px] font-black uppercase tracking-widest border"
                        :class="
                          isDark
                            ? 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20'
                    ...
                    <p class="text-xs truncate" :class="isDark ? 'text-slate-500' : 'text-slate-500'">
                      {{ group.description || $t('common.no_description') }}
                    </p>
                  </div>
                  ...
                  <div class="hidden md:flex flex-col items-center px-6 border-x border-var-border">
                    <span
                      ...
                      :class="isDark ? 'text-slate-300' : 'text-slate-900'"
                      >{{ Object.keys(group.apis || {}).length }}</span
                    >
                    <span class="text-[9px] font-bold text-slate-400 dark:text-slate-600 uppercase mt-1">{{
                      $t('common.endpoints')
                    }}</span>
                  </div>
                ...
                <transition name="slide-down">
                  <div
                    v-if="expandedGroups.has(grpKey)"
                    class="border-t border-var-border p-5 space-y-2"
                    ...
                    <div
                      ...
                      :class="
                        isDark
                          ...
                          : 'bg-white border-slate-100 hover:shadow shadow-sm'
                      "
                    >
                          ...
                          :class="
                            api.enabled
                              ...
                              : isDark
                                ...
                                : 'bg-slate-300'
                          "
                          class="w-1.5 h-1.5 rounded-full flex-shrink-0 shadow-sm"
                        ></div>
                        <div class="min-w-0">
                          <div class="flex items-center gap-2">
                            <span
                              ...
                              :class="isDark ? 'text-slate-300' : 'text-slate-900'"
                              >{{ api.alias || api.name }}</span
                            >
                            <span v-if="api.alias || api.name" class="text-[9px] font-mono text-slate-500 opacity-60"
                              >({{ apiId }})</span
                            >
                            <span
                              v-else
                          ...
                          <div class="text-[9px] font-mono text-slate-400 truncate tracking-tight">
                            {{ api.url || apiId }}
                          </div>
                        </div>
                      </div>
                      ...
                      <div class="flex items-center gap-4 ml-4">
                        ...
                        <div class="hidden sm:flex items-center gap-3 pr-4 border-r border-var-border">
                          <div
                            v-if="api.requireLogin"
                            class="p-1 rounded bg-amber-500/10 text-amber-500"
                            :title="$t('common.require_login_tip')"
                          >
                            <Lock class="w-3 h-3" />
                          </div>
                          <div
                            v-if="api.allowIps?.length"
                            class="p-1 rounded bg-primary/10 text-primary text-[9px] font-bold px-1.5"
                          >
                            IP: {{ api.allowIps.length }}
                          </div>
                          <div
                            v-if="api.allowRoles?.length"
                        ...
                        <div class="flex items-center gap-2">
                          <button
                            @click="$emit('toggle', grpKey, apiId)"
                          ...
                          <button
                            @click="$emit('edit', activeSystem, grpKey, apiId)"
