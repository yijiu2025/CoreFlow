    ...
    >
      <div class="flex justify-between items-start mb-2">
        <div class="flex flex-col min-w-0">
          ...
          <span
            class="text-xs font-mono font-bold truncate pr-2"
            :class="isDark ? 'text-slate-200' : 'text-slate-900'"
            >{{ simplifyPath(item.path) }}</span
          >
        </div>
        <div class="text-right">
          <span
            class="text-[10px] font-bold uppercase block mb-0.5"
            :class="isDark ? 'text-slate-500' : 'text-slate-400'"
            >{{ $t('stats.hits') }}</span
          >
          <span
      ...
      <div
        class="h-1 w-full rounded-full overflow-hidden transition-colors duration-500"
