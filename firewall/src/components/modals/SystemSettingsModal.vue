<template>
  ...
  >
    ...
    <template #header-actions>
      <div
        class="flex items-center gap-2 px-3 py-1 rounded-full border transition-colors duration-500"
    ...
    <div class="flex-1 flex overflow-hidden h-full">
      <!-- Sidebar Navigation -->
      <div
        class="w-72 flex flex-col border-r transition-colors shrink-0"
      ...
      <div class="flex-1 flex flex-col bg-transparent overflow-hidden h-full">
        <div class="flex-1 overflow-y-auto px-10 py-10 custom-scrollbar space-y-8 h-full">
          ...
          <div v-if="activeTab === 'panel'" class="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <h3 class="text-xs font-black uppercase tracking-widest text-slate-500 px-4 mb-4">
              {{ $t('settings.panel.node_ui') }}
            </h3>
            ...
            <div class="space-y-2">
              <SettingRow
                :is-dark="isDarkMode"
                :icon="Monitor"
                :title="$t('settings.node.name') || '节点名称'"
                description="Identify this instance"
              >
                <input
                  v-model="serverPosition.name"
                  type="text"
                  @blur="handleSyncNode"
                  class="w-64 rounded-xl px-4 py-2.5 text-sm font-bold outline-none transition-all border text-right"
                  :class="
                    isDarkMode
                      ? 'bg-black/40 border-white/10 text-white focus:border-indigo-500/50'
                      : 'bg-white border-slate-200 text-slate-900 shadow-sm focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10'
                  "
                />
              </SettingRow>

              <div
                class="p-6 rounded-2xl border transition-colors duration-300 space-y-4"
                ...
                <div class="flex items-center gap-4">
                  ...
                  <div>
                    <p class="text-sm font-bold" :class="isDarkMode ? 'text-white' : 'text-slate-900'">
                      {{ $t('settings.panel.node_location') }}
                    </p>
                    <p class="text-[10px] text-slate-500 font-bold uppercase tracking-widest">
                      {{ $t('settings.panel.node_location_desc') }}
                    </p>
                  </div>
                </div>
                <div class="grid grid-cols-3 gap-3 pl-14">
                  <FormField
                    v-model="serverPosition.country"
                    :label="$t('settings.panel.country')"
                    :is-dark="isDarkMode"
                    placeholder="China"
                    @blur="handleSyncNode"
                  />
                  <FormField
                    v-model="serverPosition.region"
                  ...
                  />
                  <FormField
                    v-model="serverPosition.city"
                    :label="$t('settings.panel.city')"
                    :is-dark="isDarkMode"
                    placeholder="Zhengzhou"
                    @blur="handleSyncNode"
                  />
                </div>
                <div class="grid grid-cols-2 gap-3 pl-14">
                  <FormField
                    v-model.number="serverPosition.lat"
                    :label="$t('settings.panel.latitude')"
                    type="number"
                    step="0.01"
                    :is-dark="isDarkMode"
                    placeholder="34.75"
                  ...
                  />
                  <FormField
                    v-model.number="serverPosition.lon"
                    :label="$t('settings.panel.longitude')"
                    type="number"
                    step="0.01"
                    :is-dark="isDarkMode"
                    placeholder="113.65"
              ...
              <SettingRow
                :is-dark="isDarkMode"
                :icon="Globe"
                :title="$t('settings.node.ip_api') || '定位 API 源'"
                description="Data provider for IPs"
              >
                <div class="relative" ref="apiSelectRef">
                  <button
                    @click="apiDropdownOpen = !apiDropdownOpen"
                    type="button"
                    ...
                    :class="
                      isDarkMode
                        ? 'bg-black/40 border-white/10 text-white hover:border-indigo-500/30'
                        : 'bg-white border-slate-200 text-slate-900 shadow-sm hover:border-indigo-500'
                    "
                  >
                    <span>{{
                      (availableIpApis.find(a => a.id === securitySettings.activeIpApi)?.name || '').toUpperCase()
                    }}</span>
                    <ChevronDown
                      class="w-4 h-4 opacity-50 transition-transform"
                      :class="apiDropdownOpen ? 'rotate-180' : ''"
                    />
                  </button>
                  <transition
                  ...
                  >
                    ...
                    >
                      <button
                        v-for="api in availableIpApis"
                        :key="api.id"
                        type="button"
                        @click="
                          securitySettings.activeIpApi = api.id;
                          handleSavePartial({ activeIpApi: api.id });
              ...
              <SettingRow
                :is-dark="isDarkMode"
                :icon="RefreshCw"
                :title="$t('settings.panel.auto_detect')"
                :description="$t('settings.panel.auto_detect_desc')"
              ...
              <SettingRow
                :is-dark="isDarkMode"
                :icon="Languages"
                :title="$t('settings.system.lang') || '系统语言'"
                description="Interface language"
              >
                <div class="flex gap-2">
                  <button
                    v-for="l in ['zh', 'en', 'ja', 'fr', 'de']"
                    :key="l"
                    @click="uiStore.setLocale(l)"
              ...
              <SettingRow
                :is-dark="isDarkMode"
                :icon="Activity"
                :title="$t('settings.panel.show_trajectory')"
          ...
          <div v-if="activeTab === 'api'" class="space-y-8 animate-in fade-in duration-300">
            <div
              class="p-10 rounded-[40px] border flex flex-col items-center text-center space-y-6 transition-all duration-300"
              :class="
                isDarkMode ? 'bg-slate-950/40 border-white/5 shadow-2xl' : 'bg-slate-50 border-slate-200 shadow-sm'
              "
            >
              <div
                class="w-20 h-20 rounded-[28px] bg-indigo-500/10 flex items-center justify-center text-indigo-500 shadow-inner"
              >
                <ShieldCheck class="w-10 h-10" />
              </div>
              <div class="space-y-2">
                <h4 class="text-2xl font-black tracking-tight" :class="isDarkMode ? 'text-white' : 'text-slate-900'">
                  {{ $t('settings.panel.api_matrix') }}
                </h4>
                <p class="text-xs text-slate-500 max-w-sm leading-relaxed font-bold uppercase tracking-wider">
                  {{ $t('settings.panel.api_matrix_desc') }}
                </p>
              </div>
          ...
          <div v-if="activeTab === 'firewall'" class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            ...
            <div class="grid grid-cols-3 gap-3">
              ...
              >
                <div class="flex items-center gap-3 min-w-0">
                  ...
                  <div class="min-w-0">
                    <p class="text-xs font-bold truncate" :class="isDarkMode ? 'text-white' : 'text-slate-900'">
                      {{ mod.label }}
                    </p>
                ...
                <div class="flex items-center gap-2 flex-shrink-0">
                  <input
                    type="checkbox"
                    v-model="securitySettings.defense[mod.key]"
                    class="toggle-switch"
                    @click.stop
                  ...
                  <ChevronRight
                    ...
                    :class="expandedSection === mod.key ? 'rotate-90' : ''"
                  />
                </div>
              </div>
            </div>
            ...
            <div class="space-y-2">
              <div
                v-if="expandedSection === 'enableRateLimit'"
                class="p-5 rounded-2xl border animate-in fade-in duration-200"
                :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <span
                    ...
                    >{{ $t('settings.firewall.rate_limit_title') }}</span
                  >
                </div>
                <div class="grid grid-cols-2 gap-4">
                  <FormField
                    v-model.number="securitySettings.defense.rateLimitRequests"
                    :label="$t('settings.firewall.max_requests')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.rateLimitWindow"
                    :label="$t('settings.firewall.time_window')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                </div>
              ...
              <div
                v-if="expandedSection === 'enableAutoBlacklist'"
                class="p-5 rounded-2xl border animate-in fade-in duration-200"
                :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.firewall.scanner_title') }}</span
                  >
                </div>
                <div class="grid grid-cols-3 gap-4">
                  <FormField
                    v-model.number="securitySettings.defense.maxNotFoundAttempts"
                    :label="$t('settings.firewall.trigger_count')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.blacklistDuration"
                    :label="$t('settings.firewall.block_duration')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.notFoundWindow"
                    :label="$t('settings.firewall.detect_window')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                </div>
              ...
              <div
                v-if="expandedSection === 'enableBruteForce'"
                class="p-5 rounded-2xl border animate-in fade-in duration-200"
                :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.firewall.brute_title') }}</span
                  >
                </div>
                <div class="grid grid-cols-3 gap-4">
                  <FormField
                    v-model.number="securitySettings.defense.bruteLimit"
                    :label="$t('settings.firewall.account_limit')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.bruteWindow"
                    :label="$t('settings.firewall.detect_window')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.bruteIpLimit"
                    :label="$t('settings.firewall.ip_limit')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.accountLockTime"
                    :label="$t('settings.firewall.account_lock')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.ipBlockTime"
                    :label="$t('settings.firewall.ip_block')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                </div>
              ...
              <div
                v-if="expandedSection === 'enableConnLimit'"
                class="p-5 rounded-2xl border animate-in fade-in duration-200"
                :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <div class="flex items-center gap-3 mb-4">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.firewall.conn_title') }}</span
                  >
                </div>
                <div class="grid grid-cols-1 gap-4 max-w-xs">
                  <FormField
                    v-model.number="securitySettings.defense.maxConn"
                    :label="$t('settings.firewall.max_conn')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                </div>
              ...
              <div
                v-if="expandedSection === 'enableGeoFilter'"
                class="p-5 rounded-2xl border animate-in fade-in duration-200 space-y-4"
                :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <div class="flex items-center gap-3">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.firewall.geo_title') }}</span
                  >
                </div>
                <FormField
                  v-model="sensitivePathsFormatted"
                  type="textarea"
                  :label="$t('settings.firewall.sensitive_paths')"
                  :is-dark="isDarkMode"
                  variant="mono"
                  :rows="3"
                />
                <div class="grid grid-cols-3 gap-4">
                  <FormField
                    v-model.number="securitySettings.defense.geoRules.overseasLimit"
                    :label="$t('settings.firewall.overseas_limit')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.geoRules.overseasWindow"
                    :label="$t('settings.firewall.overseas_window')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.geoRules.overseasBlockTime"
                    :label="$t('settings.firewall.overseas_block')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                </div>
              ...
              <div
                v-if="expandedSection === 'enableBotChallenge'"
                class="p-5 rounded-2xl border animate-in fade-in duration-200 space-y-4"
                :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <div class="flex items-center gap-3">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.firewall.bot_title') }}</span
                  >
                </div>
                <div class="grid grid-cols-2 gap-4">
                  <FormField
                    v-model.number="securitySettings.defense.botChallengeNoUaLimit"
                    :label="$t('settings.firewall.no_ua_threshold')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.botChallengeBotLimit"
                    :label="$t('settings.firewall.bot_ua_threshold')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                  <FormField
                    v-model.number="securitySettings.defense.botChallengeBrowserLimit"
                    :label="$t('settings.firewall.browser_threshold')"
                    type="number"
                    :is-dark="isDarkMode"
                  />
                </div>
                <div class="grid grid-cols-2 gap-4">
                  <FormField
                    v-model="botPatternsFormatted"
                    type="textarea"
                    :label="$t('settings.firewall.bot_ua_keywords')"
                    :is-dark="isDarkMode"
                    variant="mono"
                    :rows="4"
                  />
                  <FormField
                    v-model="browserPatternsFormatted"
                    type="textarea"
                    :label="$t('settings.firewall.browser_ua_keywords')"
                    :is-dark="isDarkMode"
                    variant="mono"
                    :rows="4"
                  />
            ...
            <!-- Network trust lists -->
            <div
              class="p-6 rounded-2xl border transition-colors duration-300"
              :class="isDarkMode ? 'bg-white/5 border-white/5' : 'bg-slate-50 border-slate-100'"
            >
              <div class="flex items-center gap-3 mb-5">
                ...
                <span
                  class="text-xs font-black uppercase tracking-widest"
                  :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                  >{{ $t('settings.firewall.trust_list') }}</span
                >
              </div>
              <div class="space-y-5">
                <TagInput
                  v-model="securitySettings.defense.internalIpPrefixes"
                  :label="$t('settings.firewall.internal_prefix')"
                  badge="IPv4/CIDR"
                  :is-dark="isDarkMode"
                  color="indigo"
                ...
                />
                <TagInput
                  v-model="securitySettings.defense.idcIpPrefixes"
                  :label="$t('settings.firewall.idc_prefix')"
                  badge="Cloud/IDC"
                  :is-dark="isDarkMode"
                  color="rose"
                ...
                />
                <TagInput
                  v-model="securitySettings.defense.safePaths"
                  :label="$t('settings.firewall.safe_paths')"
                  badge="Paths"
                  :is-dark="isDarkMode"
                  color="emerald"
              ...
              <div class="mt-4 flex justify-end">
                <PrimaryButton
                  @click="
                    handleSavePartial({
                      internalIpPrefixes: securitySettings.defense.internalIpPrefixes,
                      idcIpPrefixes: securitySettings.defense.idcIpPrefixes,
                      safePaths: securitySettings.defense.safePaths
                    })
                  "
                  variant="indigo"
                  size="sm"
          ...
          <div v-if="activeTab === 'others'" class="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div class="grid grid-cols-1 gap-3">
              <SettingRow
                :is-dark="isDarkMode"
                :icon="RefreshCw"
                :title="$t('settings.others.full_sync')"
                :description="$t('settings.others.full_sync_desc')"
                icon-bg="bg-emerald-500/10"
                icon-color="text-emerald-400"
              >
                <PrimaryButton @click="handleFetchData()" variant="emerald" size="sm">{{
                  $t('settings.others.execute_sync')
                }}</PrimaryButton>
              </SettingRow>
              ...
              <SettingRow
                :is-dark="isDarkMode"
                :icon="Trash2"
                :title="$t('settings.others.audit_reset')"
                :description="$t('settings.others.audit_reset_desc')"
                icon-bg="bg-rose-500/10"
                icon-color="text-rose-500"
              >
                <PrimaryButton @click="handleResetStats()" variant="rose" size="sm">{{
                  $t('settings.others.execute_reset')
                }}</PrimaryButton>
              </SettingRow>
            </div>
            ...
            >
              <div
                class="flex items-center justify-between border-b pb-6 mb-6"
                :class="isDarkMode ? 'border-white/5' : 'border-slate-200'"
              >
                <div class="flex items-center gap-3">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.others.ban_management') }}</span
                  >
                  <span
                    class="text-[9px] font-black px-2 py-0.5 rounded-full"
                    :class="isDarkMode ? 'bg-rose-500/10 text-rose-400' : 'bg-rose-100 text-rose-600'"
                    >{{ activeBlocks.length }}</span
                  >
                </div>
                <button
                  @click="defenseStore.fetchBlocks()"
                  class="text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg transition-colors"
                  :class="
                    isDarkMode
              ...
              <div class="flex gap-4 items-end mb-6">
                <div class="w-28">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2 block px-1"
                    >Type</label
                  >
                  <select
                ...
                <div class="flex-1">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2 block px-1">{{
                    newBlockType === 'fingerprint' ? 'Fingerprint' : $t('settings.others.ip_address')
                  }}</label>
                  <input
                    v-model="newBlockIp"
                    type="text"
                    :placeholder="newBlockType === 'fingerprint' ? 'a1b2c3d4e5f6...' : '192.168.1.1'"
                    class="w-full rounded-xl px-4 py-3 text-sm font-mono font-bold outline-none transition-all border"
                    :class="
                      isDarkMode
                        ? 'bg-black/40 border-white/10 text-white focus:border-indigo-500/50'
                        : 'bg-white border-slate-200 text-slate-900 shadow-sm focus:border-indigo-500'
                    "
                  />
                </div>
                <div class="w-32">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2 block px-1">{{
                    $t('settings.others.duration_sec')
                  }}</label>
                  <input
                    v-model.number="newBlockDuration"
                    type="number"
                    :disabled="newBlockPermanent"
                    placeholder="86400"
                ...
                <button
                  @click="newBlockPermanent = !newBlockPermanent"
              ...
              <DataTable
                ...
                :data="activeBlocks"
                :empty-message="$t('settings.others.no_active_bans')"
              >
                <template #cell-target="{ row }">
                  <span
                    class="font-mono text-xs font-bold"
                    :class="isDarkMode ? 'text-emerald-400' : 'text-emerald-600'"
                    >{{ row.type === 'fingerprint' ? row.fingerprint : row.ip }}</span
                  >
                </template>
                <template #cell-type="{ row }">
                  <span
                    class="text-[9px] font-black uppercase px-2 py-0.5 rounded-md"
                    :class="
                ...
                <template #cell-expire="{ row }">
                  <span
                    v-if="row.permanent"
                    class="text-[9px] font-black uppercase px-2 py-0.5 rounded-md bg-rose-500/10 text-rose-400"
                    >{{ $t('common.permanent') }}</span
                  >
                  <span
                    v-else
                    class="text-[10px] font-bold"
                    :class="isDarkMode ? 'text-slate-400' : 'text-slate-500'"
                    >{{ formatRemaining(row.remainingSeconds) }}</span
                  >
                </template>
                <template #cell-action="{ row }">
                  <button
                    @click="
                      handleRemoveBlock({
                        type: row.type,
                        value: row.type === 'fingerprint' ? row.fingerprint : row.ip
                      })
                    "
            ...
            >
              ...
              >
                <div class="flex items-center gap-3">
                  ...
                  <span
                    class="text-xs font-black uppercase tracking-widest"
                    :class="isDarkMode ? 'text-white' : 'text-slate-900'"
                    >{{ $t('settings.others.whitelist_management') }}</span
                  >
                  <span
                    class="text-[9px] font-black px-2 py-0.5 rounded-full"
                    :class="isDarkMode ? 'bg-emerald-500/10 text-emerald-400' : 'bg-emerald-100 text-emerald-600'"
                    >{{ activeWhitelist.length }}</span
                  >
                </div>
                <button
                  @click="defenseStore.fetchWhitelist()"
                  class="text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-lg transition-colors"
                  :class="
                    isDarkMode
                      ? 'text-slate-400 hover:text-white hover:bg-white/5'
              ...
              <div class="flex gap-4 items-end mb-6">
                ...
                <div class="flex-1">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2 block px-1">{{
                    newWlType === 'fingerprint' ? 'Fingerprint' : $t('settings.others.ip_address')
                  }}</label>
                  <input
                    v-model="newWlIp"
                    type="text"
                    :placeholder="newWlType === 'fingerprint' ? 'a1b2c3d4e5f6...' : '192.168.1.1'"
                    class="w-full rounded-xl px-4 py-3 text-sm font-mono font-bold outline-none transition-all border"
                    :class="
                      isDarkMode
                ...
                <div class="w-40">
                  <label class="text-[10px] font-black text-slate-400 uppercase tracking-[0.15em] mb-2 block px-1">{{
                    $t('settings.others.whitelist_duration')
                  }}</label>
                  <input
              ...
              <DataTable
                ...
                :data="activeWhitelist"
                :empty-message="$t('settings.others.no_active_whitelist')"
              >
                <template #cell-target="{ row }">
                  <span
                    class="font-mono text-xs font-bold"
                ...
                <template #cell-remaining="{ row }">
                  <span class="text-[10px] font-bold" :class="isDarkMode ? 'text-slate-400' : 'text-slate-500'">{{
...
const botPatternsFormatted = computed({
  ...
  set: val => {
    if (securitySettings.value?.defense) {
      securitySettings.value.defense.botPatterns = val
        .split('\n')
        .map(s => s.trim())
        .filter(Boolean);
    }
...
const browserPatternsFormatted = computed({
  ...
  set: val => {
    if (securitySettings.value?.defense) {
      securitySettings.value.defense.browserPatterns = val
        .split('\n')
        .map(s => s.trim())
        .filter(Boolean);
    }
...
const sensitivePathsFormatted = computed({
  ...
  set: val => {
    if (securitySettings.value?.defense?.geoRules) {
      securitySettings.value.defense.geoRules.sensitivePaths = val
        .split('\n')
        .map(s => s.trim())
        .filter(Boolean);
    }
...
const defenseModules = computed(() => [
  {
    key: 'enableRateLimit',
    label: t('settings.firewall.defense_modules.rate_limit'),
    sub: 'Rate Limit',
    icon: Activity,
  ...
  {
    key: 'enableAutoBlacklist',
    label: t('settings.firewall.defense_modules.scanner'),
    sub: 'Auto Block',
    icon: Bug,
  ...
  {
    key: 'enableBruteForce',
    label: t('settings.firewall.defense_modules.brute_force'),
    sub: 'Brute Force',
    icon: Lock,
  ...
  {
    key: 'enableConnLimit',
    label: t('settings.firewall.defense_modules.conn_limit'),
    sub: 'Conn Limit',
    icon: Wifi,
  ...
  {
    key: 'enableGeoFilter',
    label: t('settings.firewall.defense_modules.geo_filter'),
    sub: 'Geo Filter',
    icon: Globe,
  ...
  {
    key: 'enableBotChallenge',
    label: t('settings.firewall.defense_modules.bot_detect'),
    sub: 'Bot Detect',
    icon: Shield,
