<template>
  ...
  >
      ...
      <div class="flex items-center gap-5">
        ...
        <div>
          ...
          <h2 class="text-2xl font-bold" :class="isDarkMode ? 'text-white' : 'text-slate-900'">
            {{ editingApi || editingGroup || editingSystem }}
          </h2>
        </div>
      </div>
    </template>
    ...
    <div class="flex-1 overflow-y-auto custom-scrollbar p-8 space-y-8">
      <!-- Node Info -->
      <div
        class="p-5 rounded-2xl border transition-colors duration-500"
        :class="isDarkMode ? 'bg-cyan-500/5 border-cyan-500/10' : 'bg-indigo-50/50 border-indigo-100/50'"
      >
        <h3
          class="text-[10px] font-bold uppercase tracking-widest mb-2 flex items-center gap-2"
        ...
        <p class="text-sm leading-relaxed italic" :class="isDarkMode ? 'text-slate-400' : 'text-slate-600'">
          "{{ form.description || $t('config.desc_empty') }}"
        </p>
      </div>
      ...
      <div class="space-y-6">
        ...
        <FormField
          v-model="form.ips"
          type="textarea"
          :label="$t('config.ip_white')"
          :is-dark="isDarkMode"
          variant="mono"
          :placeholder="$t('config.ip_placeholder')"
          :rows="3"
          :hint="$t('config.ip_help')"
        >
          <template #label-icon>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div class="space-y-3">
            ...
            <div
              @click="form.requireLogin = !form.requireLogin"
              :class="[
                form.requireLogin
                  ? isDarkMode
            ...
            >
              <span class="text-xs font-bold" :class="form.requireLogin ? 'text-amber-500' : 'text-slate-500'">{{
                $t('config.require_login')
              }}</span>
    ...
    <template #footer>
      <div
        ...
        :class="isDarkMode ? 'text-slate-500' : 'text-slate-400'"
      >
        <RotateCcw class="w-3 h-3" /> {{ $t('common.last_update') }}:
        {{ form.updatedAt ? new Date(form.updatedAt).toLocaleString() : $t('common.never') }}
      </div>
