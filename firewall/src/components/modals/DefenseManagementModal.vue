<template>
  ...
  >
    ...
    <template #header-actions>
      ...
      >
        ...
        <span class="text-[9px] font-bold uppercase" :class="isDarkMode ? 'text-cyan-400' : 'text-green-600'">{{
          $t('settings.defense.status_active')
        }}</span>
      </div>
    </template>
    ...
    <div class="flex-1 flex overflow-hidden h-full">
      ...
      <div
        class="w-64 border-r flex flex-col transition-colors shrink-0"
      ...
      >
        ...
        <div class="mt-auto p-6 border-t border-white/5" :class="isDarkMode ? 'bg-black/20' : 'bg-slate-100/50'">
          ...
          <p class="text-[10px] text-slate-500 leading-relaxed font-mono font-bold">
            {{ summary.totalRequests > 0 ? ((summary.totalBlocked / summary.totalRequests) * 100).toFixed(1) : '0.0' }}%
            FILTER EFFICIENCY
          </p>
        </div>
      </div>
      ...
      <div class="flex-1 flex flex-col min-w-0 bg-transparent relative overflow-hidden h-full">
        <!-- Header -->
        <header
          class="px-10 py-8 border-b transition-colors shrink-0"
        ...
        >
          <div class="flex items-start justify-between">
            ...
            <div class="flex items-center gap-4">
              <div
                class="flex items-center gap-3 px-4 py-2 rounded-2xl border transition-all"
                :class="isDarkMode ? 'bg-slate-900 border-white/5' : 'bg-slate-50 border-slate-100'"
              >
                <span
                  class="text-xs font-bold"
                  :class="
                    isRuleEnabled(activeSubTab) ? (isDarkMode ? 'text-cyan-400' : 'text-green-600') : 'text-slate-400'
                  "
                >
                  {{ isRuleEnabled(activeSubTab) ? $t('common.sys_running') : $t('common.sys_disabled') }}
                </span>
                <ToggleSwitch
                  :model-value="isRuleEnabled(activeSubTab)"
        ...
        <div class="flex-1 overflow-y-auto p-10 space-y-12 custom-scrollbar">
          <section>
            <div class="flex items-center justify-between mb-6">
              <h3
                ...
                :class="isDarkMode ? 'text-slate-300' : 'text-slate-900'"
              >
            ...
            <div class="grid grid-cols-1 gap-3">
              ...
              <template v-if="activeSubTab === 'scanning'">
                <SettingRow
                  :is-dark="isDarkMode"
                  :icon="ShieldAlert"
                  :title="$t('settings.defense.trap_trigger')"
                ...
                <SettingRow
                  :is-dark="isDarkMode"
                  :icon="Lock"
                  :title="$t('settings.defense.block_time')"
              ...
              <template v-else-if="activeSubTab === 'rate'">
                <SettingRow
                  :is-dark="isDarkMode"
                  :icon="Activity"
                  :title="$t('settings.defense.rate_reqs')"
          ...
          <!-- Deploy Section -->
          <div
            class="flex items-center justify-between p-8 rounded-2xl transition-colors"
            :class="
              isDarkMode
                ? 'bg-cyan-500/5 border border-cyan-500/10 shadow-lg shadow-cyan-500/5'
...
import { ref } from 'vue';
import { ShieldCheck, ShieldAlert, Zap, Activity, Lock, Monitor, Target, Info, LayoutGrid } from 'lucide-vue-next';
