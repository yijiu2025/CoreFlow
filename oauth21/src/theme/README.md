...
### 🔴 明暗（`mode`）与配色是**正交**的，且「深浅」不是配色的一部分

- **`mode`**（`mode.ts`：浅色 / 深色 / 跟随系统）是**系统级偏好**，全局一个值。
- **配色**是**具体的一套外观**，自带底色，选谁就是谁。

**因此不要为"深色版"另开一个 `mode` 档，也不要让配色去改 `mode`。**
需要一套深色的品牌色？**加一个新颜色目录**（如 `navy`），而不是把它做成"某配色的深色档"。
用户 2026-09-25 的原话：

> 没有深浅两档了，深和浅就是两种颜色配置。

反过来说，`black` 与 `white` 也不是"同一个配色的明暗两档"，而是**两个完全并列的颜色**（各有自己的 id、
各自出现在面板里）。选 `black` 就是黑底，选 `white` 就是白底，**与当前 `mode` 偏好无关**。

⚠️ 因此调试面板**不提供**「点颜色顺带改明暗」这种行为（曾有过 `COLOR_MODE` 映射，
2026-09-25 已删除）：那会让"选了黑再选蓝，底色还是黑"这类困惑出现。
