 ...
 *
 * ⚠️ 配色 id 的唯一性作用域是「**同包 × 同设备 × 同页面 × 同版式**」；跨设备、跨包、
 *    跨页面、跨版式都**可以同名**。`mobile/login/colors/black` 与
 *    `standard/login/colors/black` 并存是正常且必要的 —— 它们本就是两套各自独立的
 *    配色（这正是"手机端、电脑端分别配置主题"的落点）。
...
export interface MauthThemeMeta {
  /**
   * 配色 id，以目录名为准：`themes/<包>/<设备>/<页面>/[<版式>/]colors/<配色>/` → 末段目录名。
   * 只允许 `[a-z0-9-]`（它会进 `data-mauth-theme` 属性选择器）。
   * 唯一性作用域 = 「同包 × 同设备 × 同页面 × 同版式」；**跨设备 / 跨包 / 跨页面 /
   * 跨版式可以同名**（同名不算冲突：取一份配色时这四段必然同时给出）。
   */
  id: string;
  /** 中文显示名 */
  name: string;
