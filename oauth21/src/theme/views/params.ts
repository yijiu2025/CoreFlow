 ...
 *
 * === 为什么放在架构层（`theme/views/`）而不是各页文件里 ===
 * 三个页面（login / register / forgot-password）各有**一份**注册表文件，但"哪个 query
 * key 属于哪台设备"这件事与页面无关 —— 抄三份必然漂移。各页的 `pick*ViewId` 只负责
 * "拿到原始值之后怎么解析（落在哪个包、是否合法）"，取哪个键这一步交给本文件。
 *
 * === 与配色侧同形（很重要）===
 * 配色侧（`stores/theme.ts` 的 `readUrlIntent`）用同样的键形
 ...
 * @since 2026-09-26
 */
import { THEME_DEVICES, type ThemeDevice } from '../index';
