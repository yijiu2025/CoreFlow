 ...
 *   2. 主题包声明             —— `themes/<id>/index.ts` 的 `views.register`
...
/** 本页版式注册表 */
export const registerViews = createViewRegistry(variantLoaders);
 ...
 * @param source.theme 主题包声明的版式 id（见 `themes/<id>/index.ts` 的 `views.register`）
 */
export function pickRegisterViewId(source: { url?: unknown; theme?: unknown } = {}): string {
  const url = asText(source.url);
  if (url) return registerViews.resolve(url) ?? registerViews.baseId;

  const env = asText(ENV_VIEW);
  return registerViews.resolve(source.theme) ?? (env ? registerViews.resolve(env) : null) ?? registerViews.baseId;
}
 ...
 */
export function preloadRegisterView(source: { url?: unknown; theme?: unknown } = {}): void {
  const id = pickRegisterViewId(source);
  if (id === registerViews.baseId) return;
  void registerViews.load(id);
}
