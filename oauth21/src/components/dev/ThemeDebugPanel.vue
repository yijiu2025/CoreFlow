...
import { DEFAULT_THEME_ID, listThemeGroups } from '@/theme';
...
/** 配色按**主题包**分组展示 —— 面板要能看出"包（版式+配色）→ 配色"这两级 */
const themeGroups = listThemeGroups();

/** 当前配色所属的主题包 —— 版式只在包内查找，所以版式列表要跟着它走 */
const activePkg = computed(() => themeStore.packageId);

/** 当前包在该页可选的版式 id（base 在最前；该页未接入或包里没实现则为空） */
const viewIds = computed(() => {
  const registry = pageViews.value?.registry;
  if (!registry) return [];
  return [registry.baseId, ...registry.list(activePkg.value)];
});
 ...
 *    版式跟随主题包，所以要在当前包内解析 —— 否则高亮会指向一个这里根本不存在的 id，
 *    `?view=typo` 时容器其实渲染的是 base，面板却高亮 typo，两边对不上。
 */
const activeView = computed(() => {
  ...
  if (fromUrl) return registry.resolve(fromUrl, pkg) ?? registry.baseId;
  const declared = page.value ? themeStore.viewFor(page.value) : undefined;
  return (declared ? registry.resolve(declared, pkg) : null) ?? registry.baseId;
});
...
<template>
  ...
  >
    ...
    <div v-if="!collapsed" class="flex-1 overflow-y-auto px-3 pb-3">
      <p class="mt-3 mb-1.5 text-[11px] font-medium text-slate-400">配色（颜色 / 圆角 / 背景图，按主题包分组）</p>
      ...
      <template v-if="pageViews && viewIds.length">
        <p class="mt-3 mb-1.5 text-[11px] font-medium text-slate-400">
          版式 · {{ pageViews.page }}（DOM 结构，取自包 {{ activePkg }}）
        </p>
        <div class="flex flex-wrap gap-1">
          <button
            v-for="id in viewIds"
