    ...
    <form @submit.prevent="handleRegister" class="flex-1 px-8 space-y-6">
      <div class="space-y-1 relative">
        <input
          ...
          class="w-full h-14 bg-white dark:bg-slate-900 rounded-2xl px-6 border border-transparent focus:border-primary/30 outline-none shadow-sm transition-all"
        />
        <span v-if="errors.nickname" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
          errors.nickname
        }}</span>
      </div>
      ...
      <div class="space-y-1 relative">
        <input
          ...
          class="w-full h-14 bg-white dark:bg-slate-900 rounded-2xl px-6 border border-transparent focus:border-primary/30 outline-none shadow-sm transition-all"
        />
        <span v-if="errors.email" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
          errors.email
        }}</span>
      </div>
      ...
      <div class="flex gap-4 items-start relative">
        ...
        <button
          ...
          class="h-14 px-6 bg-white dark:bg-slate-900 rounded-2xl font-bold text-sm text-primary shadow-sm active:scale-95 transition-all disabled:text-slate-400"
        >
      ...
      <div class="space-y-1 relative">
        <input
          ...
          class="w-full h-14 bg-white dark:bg-slate-900 rounded-2xl px-6 border border-transparent focus:border-primary/30 outline-none shadow-sm transition-all"
        />
        <span v-if="errors.confirmPassword" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
      ...
      <div class="pt-4">
        <button
          type="submit"
          class="w-full h-14 bg-gradient-to-r from-primary to-fuchsia-600 text-white rounded-2xl font-bold shadow-xl shadow-primary/20 active:scale-95 transition-all"
        >
          立即注册
        </button>
      </div>
      ...
      <label class="flex items-start gap-3 cursor-pointer py-4">
        ...
        <div
          class="mt-0.5 w-4 h-4 rounded border-2 border-slate-300 flex items-center justify-center transition-all"
        ...
        <span class="text-[11px] text-slate-400 leading-tight"
          >同意并接受《用户协议》、《隐私权保护声明》及相关配套条款</span
        >
      </label>
    </form>
