...
<template>
  <div class="flex flex-col min-h-screen w-full bg-white dark:bg-slate-950 font-sans pb-safe">
    <!-- Header Decor -->
    <div
      class="relative h-64 bg-gradient-to-br from-primary to-fuchsia-600 rounded-b-[40px] overflow-hidden flex flex-col items-center justify-center px-8"
    >
      ...
      <div
        class="relative z-10 w-16 h-16 bg-white/20 backdrop-blur-md rounded-2xl flex items-center justify-center text-white mb-4 border border-white/30 shadow-xl"
      >
    ...
    <div class="flex-1 px-8 -mt-10 relative z-20">
      <div
        class="bg-white dark:bg-slate-900 rounded-[32px] p-8 shadow-2xl shadow-primary/5 border border-slate-100 dark:border-slate-800"
      >
        <!-- Mode Switcher -->
        ...
        <form @submit.prevent="handleLogin" class="space-y-6">
          <transition name="mobile-fade" mode="out-in">
            <div v-if="loginType === 'sms'" key="sms" class="space-y-6">
              <div class="relative">
                <div
                  class="flex items-center h-14 bg-slate-50 dark:bg-slate-800/50 rounded-2xl px-5 border border-transparent focus-within:border-primary/30 focus-within:bg-white dark:focus-within:bg-slate-800 transition-all"
                >
                ...
                <span v-if="errors.phone" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
                  errors.phone
                }}</span>
              </div>
              ...
              <div class="relative">
                ...
                <span v-if="errors.code" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
                  errors.code
                }}</span>
              </div>
            </div>
            ...
            <div v-else key="pwd" class="space-y-6">
              <div class="relative">
                ...
                <span v-if="errors.username" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
                  errors.username
                }}</span>
              </div>
              ...
              <div class="relative">
                ...
                <span v-if="errors.password" class="absolute -bottom-5 left-2 text-[10px] text-destructive">{{
                  errors.password
                }}</span>
              </div>
            </div>
        ...
        <div class="mt-8">
          ...
          <label class="flex items-center gap-3 cursor-pointer mt-3 select-none">
            <input type="checkbox" v-model="keepLogin" class="hidden" />
            <div
              class="w-4 h-4 rounded-full border-2 border-slate-200 dark:border-slate-700 flex items-center justify-center transition-all"
              :class="{ 'bg-primary border-primary': keepLogin }"
            >
              <svg v-if="keepLogin" viewBox="0 0 24 24" width="10" height="10" fill="none" stroke="white" stroke-width="4">
                <polyline points="20 6 9 17 4 12"></polyline>
              </svg>
            </div>
            <span class="text-xs text-slate-400">记住登录状态（30 天免登录）</span>
          </label>
        </div>
      </div>
    </div>
    ...
    <div class="p-12 flex flex-col items-center">
      ...
      <div class="flex gap-8">
        <button
          class="w-12 h-12 rounded-full bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-300 shadow-sm border border-slate-100 dark:border-slate-700 active:scale-90 transition-transform"
        >
        ...
        >
          <svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor">
            <path
              d="M12.48 10.92v3.28h7.84c-.24 1.84-.908 3.152-1.628 3.872-1.012 1.012-2.6 2.12-5.64 2.12-4.824 0-8.54-3.896-8.54-8.72s3.716-8.72 8.54-8.72c2.608 0 4.504 1.024 5.904 2.344l2.304-2.304C19.144 2.616 16.272 1.04 12.48 1.04 5.688 1.04.08 6.648.08 13.44s5.608 12.4 12.4 12.4c3.672 0 6.448-1.216 8.64-3.496 2.264-2.264 2.984-5.424 2.984-7.976 0-.768-.064-1.496-.184-2.184h-11.44z"
            />
          </svg>
        </button>
      </div>
