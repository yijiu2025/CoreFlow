...
const consentActionUrl = ref(
  ((import.meta as any).env?.VITE_API_BASE_URL || 'http://localhost:3000') + '/authorize/consent'
);
...
<template>
  <div v-if="isInvalidAccess" class="flex flex-col items-center justify-center text-center p-8 mt-20">
    <div
      class="w-20 h-20 bg-red-100 dark:bg-red-900/30 text-red-500 rounded-[24px] flex items-center justify-center mb-6 shadow-sm"
    >
  ...
  <div v-else class="relative group">
    ...
    <div
      class="absolute -bottom-24 -right-24 w-64 h-64 bg-fuchsia-500/20 rounded-full blur-3xl animate-pulse delay-700"
    ></div>

    <div
      class="relative w-[856px] h-[480px] glass-effect rounded-[32px] overflow-hidden flex shadow-2xl shadow-primary/10 border-white/40 dark:border-slate-700/40"
    >
      <!-- Left Panel: Form -->
      <div class="flex-1 p-12 flex flex-col relative">
        ...
        <template v-if="authorizeState?.action === 'consent'">
          <div class="flex-1 flex flex-col items-center justify-center text-center">
            <div
              class="w-24 h-24 bg-gradient-to-tr from-primary to-fuchsia-500 rounded-full flex items-center justify-center text-white text-4xl font-bold shadow-lg shadow-primary/20 mb-6 overflow-hidden"
            >
              <img
                v-if="authorizeState.user?.avatar"
                :src="authorizeState.user.avatar"
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400 mb-10">
              {{ authorizeState.user?.email || '已在其他应用登录' }}
            </p>
            ...
            <button
              @click="authorizeState.action = 'login'"
              class="mt-6 text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-primary transition-colors"
            >
              使用其他账号登录
            </button>
          </div>
        ...
        <template v-else>
          ...
          <div class="flex items-center gap-4 mb-10">
            <div
              class="w-11 h-11 rounded-xl bg-gradient-to-tr from-primary to-fuchsia-500 flex items-center justify-center text-white shadow-lg shadow-primary/20"
            >
              ...
              <transition name="fade-slide" mode="out-in">
                ...
                <div v-if="loginType === 'sms'" key="sms" class="space-y-4">
                  <div class="group relative">
                    <div
                      class="flex h-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 items-center gap-3 transition-all focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary"
                    >
                    ...
                    <span v-if="errors.phone" class="absolute -bottom-5 left-1 text-[10px] text-destructive">{{
                      errors.phone
                    }}</span>
                  </div>
                  ...
                  <div class="group relative">
                    ...
                    <span v-if="errors.code" class="absolute -bottom-5 left-1 text-[10px] text-destructive">{{
                      errors.code
                    }}</span>
                  </div>
                </div>
                ...
                <div v-else key="pwd" class="space-y-4">
                  <div class="group relative">
                    ...
                    <span v-if="errors.username" class="absolute -bottom-5 left-1 text-[10px] text-destructive">{{
                      errors.username
                    }}</span>
                  </div>
                  ...
                  <div class="group relative">
                    ...
                    <span v-if="errors.password" class="absolute -bottom-5 left-1 text-[10px] text-destructive">{{
                      errors.password
                    }}</span>
                  </div>
                </div>
          ...
          <label class="flex items-start gap-2.5 cursor-pointer group/terms">
            ...
            >
              <svg v-if="agreed" viewBox="0 0 24 24" width="12" height="12" fill="none" stroke="white" stroke-width="4">
      ...
      <div
        class="w-[340px] bg-slate-50/50 dark:bg-slate-800/30 border-l border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center p-12"
      >
        ...
        <div
          class="relative p-3 bg-white dark:bg-white rounded-2xl shadow-xl shadow-slate-200/50 dark:shadow-black/20 group/qr overflow-hidden"
        >
          ...
          />
          <div
            class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 bg-slate-900 rounded-lg border-[3px] border-white flex items-center justify-center text-white shadow-lg"
          >
        ...
        <div
          class="mt-8 flex items-center gap-2 px-4 py-2 bg-white/80 dark:bg-slate-700/50 rounded-full shadow-sm text-[12px] font-medium text-slate-600 dark:text-slate-300 border border-slate-100 dark:border-slate-600"
        >
