...
async function getRedisClient() {
  ...
  client.on('error', err => log.warn(`⚠️ [Redis] ${C.yellow}连接错误: ${err.message}${C.reset}`));
    ...
    for (const pattern of patterns) {
      for await (const key of redis.scanIterator({ MATCH: pattern, COUNT: 500 })) {
        if (Date.now() > deadline) {
          log.warn(`${C.yellow}⚠️ Redis scan 超时（${SCAN_TIMEOUT / 1000}s），已扫描到的 key 继续处理${C.reset}`);
          break outer;
        }
        if (!seen.has(key)) {
          seen.add(key);
...
main()
  .catch(err => {
    log.error(`${C.red}❌ 执行出错: ${err.message}${C.reset}`);
    log.error(err.stack);
    process.exitCode = 1;
  })
  .finally(async () => {
