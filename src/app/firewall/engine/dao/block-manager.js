...
async function setBlock(redisClient, ip, metadata) {
  ...
  try {
    ...
    // Redis 写入失败，降级到内存
    log.error('[Firewall] setBlock Redis 失败，降级到内存:', err);
    memoryBlocks.set(ip, {
      status: metadata.status,
      source: metadata.source || 'auto',
      permanent: metadata.permanent || false,
...
async function removeBlock(redisClient, ip) {
  ...
  try {
    ...
    log.error('[Firewall] removeBlock Redis 失败:', err);
  }
}
...
async function setWhitelist(redisClient, ip, durationSeconds) {
  ...
  try {
    ...
    log.error('[Firewall] setWhitelist Redis 失败:', err);
  }
}
...
async function removeWhitelist(redisClient, ip) {
  ...
  try {
    ...
    log.error('[Firewall] removeWhitelist Redis 失败:', err);
  }
}
...
async function setBlockFp(redisClient, fingerprint, metadata) {
  ...
  try {
    ...
    log.error('[Firewall] setBlockFp Redis 失败，降级到内存:', err);
    memoryBlocksFp.set(fingerprint, {
      status: metadata.status,
      source: metadata.source || 'manual',
      permanent: metadata.permanent || false,
...
async function removeBlockFp(redisClient, fingerprint) {
  ...
  try {
    ...
    log.error('[Firewall] removeBlockFp Redis 失败:', err);
  }
}
...
async function setWhitelistFp(redisClient, fingerprint, durationSeconds) {
  ...
  try {
    ...
    log.error('[Firewall] setWhitelistFp Redis 失败:', err);
  }
}
...
async function removeWhitelistFp(redisClient, fingerprint) {
  ...
  try {
    ...
    log.error('[Firewall] removeWhitelistFp Redis 失败:', err);
  }
}
...
async function getActiveBlocks(redisClient) {
  ...
  if (redisClient) {
    try {
      ...
      log.error('[Firewall] getActiveBlocks Redis 失败:', err);
    }
  }
...
async function getActiveWhitelist(redisClient) {
  ...
  if (redisClient) {
    try {
      ...
      log.error('[Firewall] getActiveWhitelist Redis 失败:', err);
    }
  }
