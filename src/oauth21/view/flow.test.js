...
const codeChallenge = (v) => crypto.createHash('sha256').update(v).digest('base64url');

// ─── 动态导入服务（用于模拟内部调用） ───
const { AuthorizationService } = await import('../src/services/authorization.service.js');
...
assert(disc.body.code_challenge_methods_supported.includes('S256'), 'S256 supported');
...
console.log('\n── Client Credentials ──');
const ccAuth = 'Basic ' + Buffer.from('m2m-client-001:demo_m2m_secret').toString('base64');
const cc = await request('POST', '/token', { grant_type: 'client_credentials', scope: 'api:read' }, { Authorization: ccAuth });
...
const refresh = await request('POST', '/token', {
  grant_type: 'refresh_token', refresh_token: oldRT, client_id: 'spa-client-001',
});
...
// ── 7. UserInfo ──
console.log('\n── UserInfo ──');
const ui = await request('GET', '/userinfo', null, { Authorization: `Bearer ${tokResp.body.access_token}` });
...
// ── 8. Device Code Flow ──
console.log('\n── Device Code Flow ──');
const di = await request('POST', '/device/code', { client_id: 'm2m-client-001', scope: 'api:read' });
...
// ── 9. Revoke ──
console.log('\n── Revoke ──');
const rv = await request('POST', '/revoke', {
  token: refresh.body.refresh_token, token_type_hint: 'refresh_token',
