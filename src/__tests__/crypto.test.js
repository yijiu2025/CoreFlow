...
describe('加密模块', () => {
  ...
  describe('Cookie 签名', () => {
    // 使用 `framework/auth/cookie.js` 的真身实现，不再内联副本。
    // 副本曾与真身算法漂移（真身用 hex 摘要 + 字节长度守卫，副本用 base64url 且无
    // 长度守卫），导致测试全绿却掩盖了真身的 timingSafeEqual 崩溃缺陷
    // （AUDIT-REPORT-2026-09-12.md 🔴-1 / 🟡-5）。
    ...
    it('签名段为 64 个非 hex 字符时不抛异常，返回 null（🔴-1 回归）', () => {
    ...
    it('各类畸形输入一律返回 null 且不抛（🔴-1 回归）', () => {
      const payload = Buffer.from('sid:0').toString('base64url');
      const malformed = [
        payload + '.' + 'z'.repeat(64), // 全非 hex
        payload + '.' + 'g'.repeat(64),
        payload + '.' + 'a'.repeat(63) + 'z', // 混合
        payload + '.', // 空签名
        '@@@@.' + 'a'.repeat(64), // 非法 base64url payload
        'x'.repeat(10000) + '.' + 'y'.repeat(64), // 超长
        Buffer.from('nocolon').toString('base64url') + '.' + '0'.repeat(64)
      ];
      for (const value of malformed) {
        expect(() => verifyCookie(value)).not.toThrow();
