...
- **多主题 / 多版式开发模式（强制）**：改「页面长什么样」必须走既有机制，不得复制业务。`oauth21` 的认证页（登录 / 注册 / 重置密码）已全部接入；**手机端已完整，电脑端目前只有目录骨架**（`themes/default/web/`：三页最小占位版式 + `colors/mono/`，无消费方 —— 核心组件仍是业务与 UI 揉在一起、尚未拆成「容器 + 版式」的单体）。六条铁律：
