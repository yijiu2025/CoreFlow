...
- **多主题 / 多版式开发模式（强制）**：改「页面长什么样」必须走既有机制，不得复制业务。`oauth21` 的认证页（登录 / 注册 / 重置密码）已全部接入；**手机端已完整，电脑端目前只有目录骨架**（`themes/default/web/`：三页最小占位版式 + 每页 `colors/{black,white}/`，无消费方 —— 核心组件仍是业务与 UI 揉在一起、尚未拆成「容器 + 版式」的单体）。六条铁律：

  1. **三个正交维度 + 一个设备轴**：① token 基线 —— `assets/styles/mobile-auth.scss` 的 `mauth-*` 类与 `--mauth-*` 变量，是**所有呈现取值的唯一出口**；② 版式 `theme/themes/<包>/<设备>/<页面>/` —— 换 **DOM 结构与交互组织**，**同一套业务**（`?view=` / 主题包 `views.<page>` / `VITE_<PAGE>_VIEW` / `base`）；③ 配色 `theme/themes/<包>/<设备>/<页面>/[<版式>/]colors/<配色>/` —— 换颜色 / 圆角 / 背景图 / 字体，**同一套 DOM**（`?theme=` / 后端下发 / localStorage / 该范围兜底配色）。⚠️ **配色住在版式下**（2026-09-24 定案）：同一页的 `base` 与 `compact` 是两套 DOM，可覆写 token 集合不同 →「一个版式有几种颜色」是独立的事。⚠️ **`<设备>` 不是独立的换装维度，而是「页面身份」**（`mobile` / `web`，白名单）：手机端页与电脑端页各住各的子树、各有配色与版式，同步由 `router/index.ts` 的 `setupThemeDeviceSync`（`watch(router.currentRoute, { immediate: true })`，必须在 `main.ts` 的 `app.use(pinia)` **之后**调用 —— 写成 `router.afterEach` 会因 Pinia `activeInstance` 未建立而抛错被吞、设备永远停默认值）在启动时一次接好，**不跑视口判定**。三者可任意组合：`?theme=cyan&view=compact`。⚠️ **`themes/` 下按主题包分文件夹，文件夹名就是 id**（各层 id 都取自目录名，配色目录名即 `data-mauth-theme` 的属性值）。包内 `meta.id` 与目录名不一致时**以目录名为准**；目录名不合 `[a-z0-9-]` / 设备段不是 `mobile`·`web` / 缺 `index.ts` / 缺 `default` 导出 / 缺 `meta` 一律**静默跳过**（一个写坏的主题不该让整页起不来）→ 所以「加了主题没生效」先核这几项。🔴 **配色 id 的唯一性作用域是「同包 × 同设备 × 同页 × 同版式」**（注册表键是 `包/设备/页面/版式/配色` **五段复合键**；跨版式 / 跨页 / 跨设备**可以**同名，如 `black` 在每个作用域各一份）—— 用配色 id 单键登记会让后扫到的被当重名**静默丢弃**（真实踩过）。🔴 **五色完全并列**（2026-09-25 改）：`black` / `white` 是**自带底色的独立颜色**（各写全 token，非"零 token 基线"），与 `blue` / `cyan` / `rainbow` 同一列表、同一优先级；**明暗（`mode`）与配色独立 + 单向联动**：每套配色声明 `tone: 'light'|'dark'`（`theme/tone.ts`，必填，漏写 `vue-tsc` 报错），切明暗时跨系别跳转（白系+夜间→黑系、再切回恢复原白系，双侧记忆槽 `lightColor`/`darkColor` 落盘）；**单向**——手动点颜色不改明暗；首屏对齐只在有落盘偏好时生效，URL `?theme=` 显式指定时豁免。需要深色品牌色就**另加一个颜色目录**（如 `navy`，标 `tone: 'dark'`），**不许让点颜色顺带改明暗**（曾有的 `COLOR_MODE` 映射已删）。🔴 **跨设备系别一致**：`getThemeRecord` 在 id 不在当前作用域时按系别取同系别首套（mobile `blue`→web 无→回落 `white`），`themeId` 不随设备变。🔴 **URL 不被视口改写**（2026-09-25）：`/m/*` 与 `/<page>` **共用同一套分发器**（`view/web/<page>/index.vue`），窄屏渲染手机端容器、宽屏渲染桌面卡片，URL 永远不变（`desktopWhenWide` / `redirectMobileRouteOnWideViewport` 已删，别加回；关卡 `verify-mobile-forgot.mjs` ③ 守这条）。🔴 **主题设备跟随实际渲染的视图，不是路由**：分发器各 watch `activeForm`→`setActiveDevice`，`/login` 窄屏渲染手机端容器时主题作用域=mobile（否则面板切手机端主题无效）；`setupThemeDeviceSync` 只给基线。🔴 版式的契约 + 注册表收在 `theme/views/` 目录（`views/registry.ts` 工厂 + `views/pages.ts` 总览 + 每页 `views/<page>.ts`），与 `themes/` **平级**；基础版式是 `<设备>/<page>/index.vue`、变体是 `<设备>/<page>/<变体>/index.vue`，**没有 `base/` 这层**。不要把它们收进目录塞回 `themes/` 里
  ...
  4. **改这一层必须跑静态关卡**：`.tmp-probe/verify-theme-dirs.mjs`（301 项，四级结构目录口径 + 实现↔文档一致性 + `tokens` 扁平值）、`.tmp-probe/verify-glob-device.mjs`（17 项，读回 `import.meta.glob` 编译产物）+ `npm run type-check`；**关卡改动后要做毒丸验证**（把某版式下的 `white/` 临时改名、退回单键登记、把设备同步改回 `afterEach`、把某配色的 `tokens` 重新包进 `light: {}` —— 四者都必须报错），否则关卡可能只是恒绿的摆设
  ...
  6. **iframe 握手靠 `SSO_READY`，父 origin 白名单是单一来源**（`oauth21/src/utils/parent-origins.ts`）：`utils/parent.ts`（发）与 `composables/useParentThemeSync.ts`（收）**共用**它，改白名单只改这一处 + 各 `.env` 的 `VITE_ALLOWED_PARENT_ORIGINS`。⚠️ **只写「嵌 oauth21 的父应用」的 origin，别写 oauth21 自己的 5174/5175**（它是被嵌方，不是父应用）。🔴 **漏配的症状是「弹窗 loading 慢」而不是报错**：`SSO_READY` 被 `postToParent` 拒发 → 宿主等满自己的 **3s 兜底超时**（实测 posecraft 弹窗 3608ms；补对白名单后 719ms）。**排查任何 iframe 握手类「慢」，第一件事 grep 控制台 `[SSO] 拒绝 postMessage：父 origin 未授权`**。补充：剩余 ~800ms 属 dev 冷启动（~65 个模块 waterfall）+ 宿主侧收尾（~280ms），**不要把 `SSO_READY` 提前到 app mount** —— 收消息的监听器在 `App.vue` setup 注册、`onMounted` 才挂，提前发会让宿主回发的主题消息落空
