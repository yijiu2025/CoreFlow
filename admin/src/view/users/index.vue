    ...
    >
      <div class="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-xl w-full max-w-md border">
        ...
        <p class="text-sm text-slate-500 mb-6">
          正在为目标用户 <span class="text-primary font-medium">{{ selectedUser?.username }}</span> 配置权限。
        </p>
        ...
        <div class="space-y-4 mb-8">
          <div>
            ...
            <select
              v-model="authForm.appId"
              @change="fetchAssignableRoles"
              class="w-full px-3 py-2 border rounded-md bg-transparent focus:ring-2 focus:ring-primary/50"
            >
        ...
        <div class="flex justify-end space-x-3">
          <button
            ...
            class="px-4 py-2 text-sm border rounded-md hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
          >
            取消
          </button>
          <button
