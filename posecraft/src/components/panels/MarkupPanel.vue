<template>
  <PanelSection title="标注工具" :shortcuts="shortcuts">
    ...
    <div class="tool-grid">
      <button class="grid-btn" :class="{ active: currentTool === 'select' }" @click="$emit('setTool', 'select')">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
          <path d="M3 3l7.07 16.97 2.51-7.39 7.39-2.51L3 3z"/>
        </svg>
        <span>选择</span>
      </button>
      <button class="grid-btn" :class="{ active: currentTool === 'draw' }" @click="$emit('setTool', 'draw')">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
          <path d="M12 19l7-7 3 3-7 7-3-3z"/>
        </svg>
        <span>画笔</span>
      </button>
      <button class="grid-btn" :class="{ active: currentTool === 'eraser' }" @click="$emit('setTool', 'eraser')">
        <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round">
...
.section-label { font-size: 11px; font-weight: 600; color: #64748b; text-transform: uppercase; letter-spacing: 0.5px; margin: 16px 0 10px; }
