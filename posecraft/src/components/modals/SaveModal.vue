...
const fetchPhotonBackup = async (lat: number, lng: number) => {
  try {
    ...
    const data = await res.json()
    const backup = data.features.map((f: any) => ({
...
.modal-header h3 {
  margin: 0; font-size: 16px; font-weight: 700; color: #818cf8;
}
.close-btn {
...
.modal-input, .modal-textarea {
  ...
  color: #e2e8f0; padding: 10px 12px; border-radius: 10px; font-size: 13px;
  transition: all 0.2s;
}
.modal-input:focus, .modal-textarea:focus {
  border-color: #6366f1; outline: none; background: rgba(10, 10, 15, 0.8);
...
.loc-action-btn {
  ...
  border-radius: 10px; width: 40px; display: flex; align-items: center;
...
.location-suggestions {
  ...
  overflow-y: auto; background: rgba(10, 10, 15, 0.8);
...
.coords-badge {
  font-size: 11px; color: #818cf8; font-family: monospace;
}
.modal-tags {
  display: flex; flex-wrap: wrap; gap: 6px; padding: 8px 10px;
  background: rgba(10, 10, 15, 0.6); border: 1px solid rgba(255, 255, 255, 0.08);
  border-radius: 10px; min-height: 42px; align-items: center;
}
.tag-pill {
  display: flex; align-items: center; gap: 6px; background: rgba(99, 102, 241, 0.15);
  border: 1px solid rgba(99, 102, 241, 0.25); color: #818cf8; font-size: 11px;
  padding: 4px 8px; border-radius: 6px; font-weight: 500;
}
.tag-x {
  cursor: pointer; font-size: 12px; color: rgba(255,255,255,0.4); transition: color 0.15s;
}
.tag-x:hover { color: #f87171; }
.modal-tag-input {
  background: none; border: none; color: #e2e8f0; font-size: 12px;
  outline: none; flex: 1; min-width: 60px; padding: 2px;
}
.modal-footer {
...
.modal-btn.primary:hover {
  transform: translateY(-1px); box-shadow: 0 6px 20px rgba(99, 102, 241, 0.5);
}
.modal-btn.secondary {
...
.exif-card-header {
  display: flex; align-items: center; gap: 6px; font-size: 11px; font-weight: 600; color: #818cf8;
  text-transform: uppercase; letter-spacing: 0.5px;
}
.exif-grid {
...
.res-badge {
  ...
  color: #818cf8; font-size: 9px; padding: 1px 4px; border-radius: 4px; margin-left: 6px;
