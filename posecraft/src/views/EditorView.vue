...
const updateSelection = () => {
  ...
  if (obj) {
    ...
    if (targetObj.type === 'i-text' || targetObj.type === 'textbox' || targetObj.type === 'text') {
      const activeStyles = (targetObj.isEditing && typeof targetObj.getActiveStyles === 'function') ? (targetObj.getActiveStyles() || {}) : {}
      const textFill = activeStyles.fill || targetObj.fill
      const parsed = parseColor(textFill)
      if (parsed) currentColor.value = parsed.hex
      // 同步文字属性到面板
      textFontSize.value = activeStyles.fontSize || targetObj.fontSize || 24
      textFontFamily.value = activeStyles.fontFamily || targetObj.fontFamily || 'Arial'
      textLineHeight.value = Math.round((targetObj.lineHeight || 1.2) * 100)
      textLetterSpacing.value = Math.round((activeStyles.charSpacing !== undefined ? activeStyles.charSpacing : (targetObj.charSpacing || 0)) / 10)
      textBold.value = (activeStyles.fontWeight || targetObj.fontWeight) === 'bold'
      textItalic.value = (activeStyles.fontStyle || targetObj.fontStyle) === 'italic'
      textUnderline.value = activeStyles.underline !== undefined ? activeStyles.underline : (targetObj.underline || false)
      textStrikethrough.value = activeStyles.linethrough !== undefined ? activeStyles.linethrough : (targetObj.linethrough || false)
