<template>
  <div class="editor-root">
    <!-- 顶部导航栏子组件 -->
    <TopBar @exit="triggerExit" @undo="undo" @redo="redo" @help="showHelp = true" @save="saveTemplate" />
    ...
    <div class="main-content">
      ...
      <RightPanel
        ...
        @update:lineHeight="
          v => {
            textLineHeight = v;
            updateTextProperty('lineHeight', v / 100);
          }
        "
        @update:letterSpacing="
          v => {
            textLetterSpacing = v;
            updateTextProperty('charSpacing', v * 10);
          }
        "
        @update:bold="
          v => {
            textBold = v;
            updateTextProperty('fontWeight', v ? 'bold' : 'normal');
          }
        "
        @update:italic="
          v => {
            textItalic = v;
            updateTextProperty('fontStyle', v ? 'italic' : 'normal');
          }
        "
        @update:underline="
          v => {
            textUnderline = v;
            updateTextProperty('underline', v);
          }
        "
        @update:strikethrough="
          v => {
            textStrikethrough = v;
            updateTextProperty('linethrough', v);
          }
        "
        @update:textAlign="
          v => {
            textAlign = v;
            updateTextProperty('textAlign', v);
          }
        "
        @zoomIn="zoomIn"
        @zoomOut="zoomOut"
    ...
    <!-- 退出确认弹窗 -->
    <ExitModal :isOpen="showExitModal" @close="showExitModal = false" @confirm="goHome" />
  </div>
</template>
...
watch(
  () => canvasAreaRef.value?.canvasContainer,
  v => {
    if (v) canvasContainer.value = v;
  }
);
watch(
  () => canvasAreaRef.value?.fileInput,
  v => {
    if (v) fileInput.value = v;
  }
);
...
watch(
  zoomSlider,
  v => {
    canvasStore.zoomSlider = v;
  },
  { immediate: true }
);
watch(
  () => canvasStore.zoomSlider,
  v => {
    zoomSlider.value = v;
  }
);
...
/** 2. 画布历史快照管理（撤销、重做） */
const { undoStack, redoStack, saveState, undo, redo, setOnStateRestored, setOnReapplyTool } = useHistory(
  fCanvas,
  isStateSavingLocked,
  null,
  null
...
const { selectTab, selectHandTool, setDrawTool, setTool, setDeps } = useTools(
  fCanvas,
  toRef(toolStore, 'activeTool'),
  toRef(toolStore, 'canvasTool')
);
...
const {
  ...
  createPolygon,
  addArrowHead
} = useShapes(fCanvas, toRef(toolStore, 'currentColor'), toRef(toolStore, 'fillColor'));

/** 5. 九宫格与黄金比例构图参考辅助线 */
const { activeGuides, drawReference, deleteGuides, toggleGuide } = useReferenceLines(
  fCanvas,
  toRef(toolStore, 'currentColor'),
  toRef(toolStore, 'strokeWidth'),
  saveState
);
watch(
  activeGuides,
  v => {
    toolStore.activeGuides = v;
  },
  { immediate: true, deep: true }
);

/** 6. 姿势骨架节点及肢体连接线连接操作 */
const { drawPoseSkeleton, addSkeletonNode, addMidpointNode, connectNodes } = useSkeletonNodes(
  fCanvas,
  toRef(toolStore, 'currentColor'),
  saveState
);
...
const canvasDeps = Object.create(getCanvasDeps(), {
  eraserOpacity: {
    get() {
      return toolStore.eraserOpacity;
    },
    enumerable: true
  },
  eraserHardness: {
    get() {
      return toolStore.eraserHardness;
    },
    enumerable: true
  },
  eraserShape: {
    get() {
      return toolStore.eraserShape;
    },
    enumerable: true
  }
});
...
const {
  ...
  fCanvas,
  toRef(toolStore, 'currentColor'),
  toRef(canvasStore, 'detectionType'),
  toRef(canvasStore, 'bgImageUploaded'),
  toRef(canvasStore, 'loadingStep'),
  toRef(canvasStore, 'analysisComplete'),
  toRef(canvasStore, 'isAnalyzing'),
  saveState,
  drawPoseSkeleton,
  canvasDeps
);
...
const { handleMouseDown, handleMouseMove, handleMouseUp, handlePathCreated } = useMouseEvents(
  fCanvas,
  toRef(toolStore, 'canvasTool'),
  toRef(toolStore, 'activeTool'),
  toRef(toolStore, 'currentColor'),
  toRef(toolStore, 'strokeWidth'),
  toRef(toolStore, 'noFill'),
  toRef(toolStore, 'fillColor'),
  toRef(toolStore, 'strokeOpacity'),
  toRef(toolStore, 'fillOpacity'),
  toRef(toolStore, 'lineStyle'),
  toRef(toolStore, 'eraserSize'),
  textFontSize,
  toRef(toolStore, 'cornerRadius'),
  canvasDeps,
  saveState,
  addSkeletonNode,
  addMidpointNode,
...
const { showSaveModal, showExitModal, saveTemplate, triggerExit, goHome, confirmSave } = useTemplateSave(
  fCanvas,
  toRef(canvasStore, 'templateName'),
  () => canvasDeps.inkCanvas
);
...
watch(
  ...
  newVal => {
    if (fCanvas.value?.isDrawingMode && fCanvas.value.freeDrawingBrush)
      fCanvas.value.freeDrawingBrush.width = Number(newVal);
  }
);
...
watch([() => toolStore.currentColor, () => toolStore.brushOpacity], ([newColor, newOpacity]) => {
  if (fCanvas.value?.isDrawingMode && fCanvas.value.freeDrawingBrush) {
    ...
    if (toolStore.brushFeather > 0 && fCanvas.value.freeDrawingBrush.shadow)
      fCanvas.value.freeDrawingBrush.shadow.color = rgbaColor;
  }
});

watch(
  () => toolStore.brushFeather,
  newVal => {
    if (fCanvas.value?.isDrawingMode && fCanvas.value.freeDrawingBrush) {
      if (newVal > 0) {
  ...
  fCanvas.value.on('object:moving', (e: any) => {
    ...
    if (activeDragLines.length > 0) {
      activeDragLines.forEach(({ id, endpoint }) => {
        ...
        const otherNode = objs.find(
          (o: any) =>
            o.isSkeleton && o.connectedLines?.some((c: any) => (c.id || c.line) === id && c.endpoint === targetEndpoint)
        );
        if (!otherNode) return;
      ...
      lines.forEach((line: any) => {
        if (Math.hypot(line.x1 - obj.left, line.y1 - obj.top) < 20) {
          ...
          line.set({
            ...
            width: Math.abs(obj.left - x2),
            height: Math.abs(obj.top - y2)
          });
          line.setCoords();
          ...
          line.set({
            x2: obj.left,
            y2: obj.top,
            left: Math.min(x1, obj.left),
            top: Math.min(y1, obj.top),
            width: Math.abs(x1 - obj.left),
            height: Math.abs(y1 - obj.top)
          });
          line.setCoords();
        }
      });
...
const {
  ...
  cancelCrop,
  setFileInput,
  setDeps: setUploadDeps
} = useImageUpload(
  fCanvas,
  toRef(toolStore, 'activeTool'),
  toRef(canvasStore, 'bgImageUploaded'),
  toRef(canvasStore, 'bgOpacity'),
  zoomSlider,
  currentZoom,
  saveState,
  applyCanvasTransform
...
watch(
  canvasContainer,
  v => {
    if (v) setUploadDeps({ canvasContainer: v });
  },
  { immediate: true }
);
setOnStateRestored(() => {
...
setOnReapplyTool(() => {
  setDrawTool(toolStore.canvasTool);
});
...
watch(
  cropAspectRatio,
  v => {
    toolStore.cropAspectRatio = v;
  },
  { immediate: true }
);
watch(
  () => toolStore.cropAspectRatio,
  v => {
    cropAspectRatio.value = v;
  }
);
...
const deleteSelected = () => {
  fCanvas.value.getActiveObjects().forEach((o: any) => fCanvas.value.remove(o));
...
const clearCanvas = () => {
  ...
  fCanvas.value
    .getObjects()
    .slice()
    .forEach((o: any) => fCanvas.value.remove(o));
  fCanvas.value.renderAll();
  saveState();
};
