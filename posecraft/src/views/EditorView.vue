...
const autoAnalyze = async () => {
  ...
  if (type === 'all' || type === 'pose') {
    ...
    fCanvas.value.getObjects().slice().forEach((o: any) => {
      if (o.isSkeleton || (o.isAutoGenerated && o.type === 'line')) fCanvas.value.remove(o)
    })
  }
  if (type === 'all' || type === 'face' || type === 'hand') {
    // 清除面部和手部（自动标记的对象）
    fCanvas.value.getObjects().slice().forEach((o: any) => {
      if (o.isAutoGenerated && !o.isSkeleton) fCanvas.value.remove(o)
    })
  }
