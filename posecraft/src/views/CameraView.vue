...
<template>
  <div class="h-screen w-screen flex flex-col bg-black relative overflow-hidden">
    <!-- 顶部导航栏 -->
    <header
      class="absolute top-0 left-0 right-0 h-14 flex items-center px-4 z-50 bg-gradient-to-b from-black/60 to-transparent"
    >
    ...
    <div class="flex-1 relative w-full h-full">
      ...
      <div v-if="uploading" class="absolute inset-0 flex items-center justify-center bg-black/50 z-30">
        <div class="bg-black/70 text-white px-6 py-4 rounded-xl flex flex-col items-center">
          <svg
            class="animate-spin h-8 w-8 text-white mb-2"
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
          >
    ...
    <!-- 底部控制栏：只保留拍照按钮 -->
    <div
      class="absolute bottom-0 left-0 right-0 h-32 flex items-center justify-center bg-gradient-to-t from-black/80 to-transparent z-50 pb-6"
    >
      <button
        @click="handleCapture"
        :disabled="uploading"
      ...
      >
        <div
          class="w-[60px] h-[60px] bg-white rounded-full active:scale-95 transition-transform duration-200 shadow-inner"
        ></div>
      </button>
    </div>
  </div>
...
onMounted(async () => {
  ...
  if (templateId && fCanvas) {
    try {
      ...
      if (poseData.fabricData) {
        // 深度克隆并清洗数据，绝不加载任何原图
        const safeFabricData =
          typeof poseData.fabricData === 'string'
