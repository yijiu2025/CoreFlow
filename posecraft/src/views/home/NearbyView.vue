    ...
    <div class="content-container">
      ...
      <template v-else-if="filteredItems.length > 0">
        ...
        <div class="load-more-container" v-else>
          <span class="no-more-text">已加载全部附近创作</span>
        </div>
      </template>
      <template v-else>
        <div class="empty-state">
          <MapPin class="empty-icon" :size="54" />
          <div class="empty-text">附近暂时没有发布的创作</div>
        </div>
      </template>
    </div>
  </div>
...
// 切换当前导航状态
activeNav.value = 'nearby'
onActivated(() => { activeNav.value = 'nearby' })
</script>
