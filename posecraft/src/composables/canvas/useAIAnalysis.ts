...
export function useAIAnalysis(
  ...
  const ensureModelsLoaded = async () => {
    ...
    try {
      ...
      segmenter = await bodySegmentation.createSegmenter(bodySegmentation.SupportedModels.MediaPipeSelfieSegmentation, {
        runtime: 'tfjs',
        modelUrl: `${base}/models/selfie_segmentation/model.json`
      });
  ...
  const drawImageOutline = async (segs: any[], hex: string, offset: any = null) => {
    ...
    try {
      ...
      c1.width = mask.width;
      c1.height = mask.height;
      c1.getContext('2d', { willReadFrequently: true })!.putImageData(mask, 0, 0);
      const c2 = document.createElement('canvas');
      c2.width = mask.width;
      c2.height = mask.height;
      const ctx = c2.getContext('2d', { willReadFrequently: true })!;

      const bg = fCanvas.value.backgroundImage;
      if (!bg) {
        console.warn('[AI] 背景图不存在');
        return;
      }
  ...
  const drawFaceMesh = (face: any, offset: any = null) => {
    if (!face?.keypoints?.length) return;
    const kps = face.keypoints;
    const mp = (i: number) => {
      const k = kps[i];
      return k ? mapPoint({ x: k.x, y: k.y, score: 1 }, offset) : null;
    };
    const addLine = (c: string, w: number, ...ps: [number, number][]) => {
      ps.forEach(([a, b]) => {
        const pA = mp(a),
          pB = mp(b);
        if (pA && pB)
          fCanvas.value.add(
            new fabric.Line([pA.x, pA.y, pB.x, pB.y], {
              stroke: c,
              strokeWidth: w,
              selectable: false,
              evented: false,
    ...
    const addDot = (c: string, r: number, i: number) => {
      const p = mp(i);
      if (p)
        fCanvas.value.add(
          new fabric.Circle({
    ...
    const le = [33, 246, 161, 160, 159, 158, 157, 173, 133, 155, 154, 153, 145, 144, 163, 7, 33];
    for (let i = 0; i < le.length - 1; i++) addLine('#34d399', 1.5, [le[i], le[i + 1]]);
    const re = [362, 398, 384, 385, 386, 387, 388, 466, 263, 249, 390, 373, 374, 380, 381, 382, 362];
    for (let i = 0; i < re.length - 1; i++) addLine('#34d399', 1.5, [re[i], re[i + 1]]);
    addLine('#fb923c', 1.5, [168, 6], [6, 197], [197, 195], [195, 5], [5, 4], [4, 1], [1, 19]);
    addDot('#fb923c', 3, 1);
    const mo = [61, 185, 40, 39, 37, 0, 267, 269, 270, 409, 291, 375, 321, 405, 314, 17, 84, 181, 91, 146, 61];
    for (let i = 0; i < mo.length - 1; i++) addLine('#f472b6', 1.5, [mo[i], mo[i + 1]]);
  };
  ...
  const drawHandSkeleton = (hand: any, offset: any = null) => {
    ...
    conn.forEach(([a, b]) => {
      if (pts[a] && pts[b])
        fCanvas.value.add(
          new fabric.Line([pts[a].x, pts[a].y, pts[b].x, pts[b].y], {
            stroke: c,
            strokeWidth: 1.5,
            selectable: false,
            evented: false,
  ...
  const runFullAnalysis = async (src: any, offset: any = null) => {
    if (!bgImageUploaded.value || !detector) return;
    let found = false;
    ...
    try {
      if ((type === 'all' || type === 'segmentation') && segmenter) {
        ...
        try {
          ...
          if (s?.length) {
            found = true;
            await drawImageOutline(s, currentColor.value, offset);
          } else {
            console.warn('[AI] 未检测到人体轮廓');
          }
        } catch (segErr) {
      ...
      if ((type === 'all' || type === 'hand') && handDetector) {
        ...
        if (h?.length) {
          h.forEach((x: any) => drawHandSkeleton(x, offset));
          found = true;
        }
      }
      fCanvas.value.getObjects().forEach((o: any) => {
        if (o.type === 'circle' || o.type === 'line') o.bringToFront();
      });
    } catch (e) {
  ...
  const autoAnalyze = async () => {
    if (isAnalyzing.value || !bgImageUploaded.value) return;
    await ensureModelsLoaded();
    if (!detector) return;
    isAnalyzing.value = true;
    loadingStep.value = '正在分析构图...';
  ...
  const analyzeArea = async (rect: any) => {
    ...
    try {
      const bg = fCanvas.value.backgroundImage,
        el = bg.getElement();
      const cx = (rect.left - (bg.left - bg.getScaledWidth() / 2)) / bg.scaleX,
        cy = (rect.top - (bg.top - bg.getScaledHeight() / 2)) / bg.scaleY;
      const cw = rect.width / bg.scaleX,
        ch = rect.height / bg.scaleY;
      const tmp = document.createElement('canvas');
      tmp.width = cw;
      tmp.height = ch;
      tmp.getContext('2d', { willReadFrequently: true })!.drawImage(el, cx, cy, cw, ch, 0, 0, cw, ch);
