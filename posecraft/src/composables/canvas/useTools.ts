...
export function useTools(fCanvas: Ref<any>, activeTool: Ref<string>, canvasTool: Ref<string>) {
  ...
  const setDeps = (deps: {
    eraserCursor?: any;
    eraserSize?: Ref<number> | null;
    brushSize?: Ref<number> | null;
    brushOpacity?: Ref<number> | null;
    brushFeather?: Ref<number> | null;
    brushStyle?: Ref<string> | null;
    brushBlend?: Ref<string> | null;
  ...
  const setDrawTool = (tool: string) => {
    ...
    if (tool === 'draw' && brushSize && currentColor) {
      ...
      // 虚线样式
      if (brushStyle) {
        fCanvas.value.freeDrawingBrush.strokeDashArray =
          brushStyle.value === 'dashed' ? [10, 5] : brushStyle.value === 'dotted' ? [3, 5] : undefined;
      }
      // 混合模式
