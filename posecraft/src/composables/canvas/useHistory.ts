...
export function useHistory(fCanvas: Ref<any>, isStateSavingLocked: { value: boolean }, inkCanvas: HTMLCanvasElement | null, inkCtx: CanvasRenderingContext2D | null) {
  ...
  const undoStack = computed({
    ...
    set: (val) => { historyStore.undoStack = val }
  })
  const redoStack = computed({
    get: () => historyStore.redoStack,
    set: (val) => { historyStore.redoStack = val }
  })
  ...
  const saveState = () => {
    ...
    const json = fCanvas.value.toJSON(['id', 'selectable', 'evented', 'hasControls', 'hasBorders', 'strokeWidth', 'stroke', 'fill', 'radius', 'scaleX', 'scaleY', 'originX', 'originY', 'name', 'opacity', 'isInkLayer', 'isAutoGenerated', 'isSkeleton', 'connectedLines', 'isUserStroke', 'erasable', 'isGuide', 'guideType'])
