...
const log = createLogger('_tmp_fix');
const p = 'src/auth/session.js';
let s = fs.readFileSync(p, 'utf8');
const marker = "import { DEVICE_TYPE, detectDeviceType } from './device.js';";
if (!s.includes(marker)) { log.error('marker not found'); process.exit(1); }
const block = [
  '',
  '/** 认证调试开关 */',
  "const DEBUG_AUTH = process.env.DEBUG_AUTH === 'true';",
  'function _debug(...args) {',
  "  if (DEBUG_AUTH) log.stdout('[Auth Debug]', ...args);",
  '}',
  '',
...
if (!re.test(s)) { log.error('context not matched'); process.exit(1); }
s = s.replace(re, marker + '\r\n' + block);
fs.writeFileSync(p, s);
log.stdout('restored, length', s.length);
