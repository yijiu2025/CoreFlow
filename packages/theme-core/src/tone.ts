 ...
 *   🔴 明暗 ≡ 色系：「明暗切换就是切换白色和黑色这个色系，不是另一套规则」。
 *   切「暗」→ 落到黑系配色、切「明」→ 落到白系配色，两侧各自记住上一次的选择，
 *   切回来即恢复。例如：白系用 blue，切「暗」→ 自动变 black，切「明」→ 回到 blue。
 *   实现见 `oauth21/src/stores/theme.ts` 的「双侧记忆槽」。
 *
 ...
 *
 * ⚠️ 明暗与配色**保持一致**：点色卡同步把明暗设成该配色的系别（点黑卡=暗、
 *    点白卡=明），切明暗联动把配色切到目标系别 —— 两条路径互相咬合，不撕裂。
 *
 * @author yijiu2025
 */
