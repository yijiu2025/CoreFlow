...
> ## 📐 实现状态：设计稿 · 未实现（2026-09-20 更新）
>
> **本页所述的「多服务器角色裁剪」方案在代码中仍零实现。**
>
> - 文中提及的 `SERVER_ROLE`、`MODULE_*` 等环境变量在全仓**没有任何引用**，`.env.example` 里也没有；
> - 当前加载器（`src/framework/loader/registry/`）是**全量加载**，不存在按角色裁剪模块的能力。
>
> **2026-09-20 更新两处已过时的事实：**
>
> 1. 配套的 Docker 制品**已落地**（`Dockerfile` / `docker-compose.yml` / `docker/entrypoint.mjs`，
>    外加 compose 全栈冒烟流水线）→ 见 [`Docker 部署`](/deployment/docker)。
> 2. 本页原先列出的横向扩展阻断项**已全部修复**并有关卡守护：调度器分布式锁（E1）、
>    守卫配置跨实例同步（E2）、遥测与 WS 广播外置（E3）。可复跑 `npm run verify:all` 验证。
>    也就是说「多个实例同时跑同一业务」已经安全，**未完成的只是「按角色裁剪模块」**。
>
> **请勿据此规划部署。** 本页保留作**设计参考**，不代表现状。
