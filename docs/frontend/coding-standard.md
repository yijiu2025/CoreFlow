...
8. **多主题 / 多版式（强制）**：呈现必须拆成**三个正交维度 + 一个设备轴** —— ① token 基线（`mauth-*` 类 + `--mauth-*` 变量的**唯一取值出口**）、② 配色 `theme/themes/<包>/<设备>/colors/<配色>/`（换取值，同一套 DOM）、③ 版式 `theme/themes/<包>/<设备>/<page>/`（换 DOM 与交互组织，同一套业务）。`<设备>` ∈ `mobile` / `web`，是**「页面身份」而非运行时判定**（同步由 `router/index.ts` 的 `setupThemeDeviceSync` 在 `app.use(pinia)` 之后一次接好，**不跑视口判定**）—— 手机端与电脑端各住各的子树、各有配色与版式。**`themes/` 下按主题包分文件夹，文件夹名就是 id**：各层目录名只允许 `[a-z0-9-]`（设备段另受白名单限制），且包/配色目录名就是 `data-mauth-theme` 的属性值，因此**目录名是唯一真源**（包内 `meta.id` 不一致时以目录名为准）。🔴 **配色 id 唯一性作用域是「设备内」**（注册表键 `包/设备/配色`；跨设备可同名，用单键登记会让后扫到的一端被静默丢弃）。铁律两条：**业务只有一份**（状态 / 校验 / 请求 / 路由 / 浮层全在容器 `view/app/<page>/index.vue`，版式只读 `ctx`、只调 `ctx.actions`）；**外部输入一律过白名单**（版式 id 限 `[a-z0-9-]` 且当前包内已登记，未登记落 `base`；主题 token 走 `theme/runtime.ts`，刻意拒 `url()` 与 CSS 颜色名）。契约写在该页 `theme/views/<page>.ts`（纯类型）+ 容器侧 `assert<Page>Contract` 编译期自检。规则全文：[多主题 / 多版式开发模式](/frontend/multi-theme)。
