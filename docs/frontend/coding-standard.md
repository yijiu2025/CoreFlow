...
8. **多主题 / 多版式（强制）**：呈现必须拆成**三个正交维度** —— ① token 基线（`mauth-*` 类 + `--mauth-*` 变量的**唯一取值出口**）、② 配色 `theme/themes/<包>/colors/<配色>/`（换取值，同一套 DOM）、③ 版式 `theme/themes/<包>/<page>/`（换 DOM 与交互组织，同一套业务）。**`themes/` 下按主题包分文件夹，文件夹名就是 id**：目录名只允许 `[a-z0-9-]`，且就是 `data-mauth-theme` / `data-mauth-view` 的属性值，因此**目录名是唯一真源**（包内 `meta.id` 不一致时以目录名为准）。 铁律两条：**业务只有一份**（状态 / 校验 / 请求 / 路由 / 浮层全在容器 `view/app/<page>/index.vue`，版式只读 `ctx`、只调 `ctx.actions`）；**外部输入一律过白名单**（版式 id 限 `[a-z0-9-]` 且当前包内已登记，未登记落 `base`；主题 token 走 `theme/runtime.ts`，刻意拒 `url()` 与 CSS 颜色名）。契约写在该页 `theme/views/<page>.ts`（纯类型）+ 容器侧 `assert<Page>Contract` 编译期自检。规则全文：[多主题 / 多版式开发模式](/frontend/multi-theme)。
