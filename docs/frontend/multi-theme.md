...
>
> 机制实现：`oauth21/src/theme/index.ts`（主题包 + 配色注册表）、
...
| ② 配色 | `src/theme/themes/<包>/colors/<配色>/` | 颜色 / 圆角 / 描边 / 背景图 / 字体，**同一套 DOM** | `?theme=`（别名 `?skin=`）→ 后端下发 → localStorage → `default` |
| ③ 版式 | `src/theme/themes/<包>/<page>/` | **DOM 结构与交互组织**，**同一套业务** | `?view=` → 主题包 `views.<page>` → `VITE_<PAGE>_VIEW` → `base` |
...
| 想改的东西 | 动哪一层 | 参考 |
| --- | --- | --- |
| 品牌色 / 圆角 / 描边宽 / 字体栈 | ② 配色 `tokens` | `themes/default/colors/ocean/` |
| 背景图 / webfont / 伪元素装饰 | ② 配色 `theme.scss` | `themes/default/colors/ocean/` |
| 表面分层（让 header / body 透明，底色交还页面） | ② 配色 `tokens` + 画布 token | `themes/default/colors/sky/` |
| 按屏幕形态给不同值（横屏 / 矮屏） | ② 配色 `theme.scss` 的媒体查询 | `themes/default/colors/sky/` |
| **DOM 顺序、分步流程、面板组织** | ③ 版式 | `themes/default/register/compact/` |
...
```text
src/theme/
├── index.ts / types.ts              # 主题包 + 配色注册表 / 契约
...
└── themes/                          # 主题包：一套完整设计 = 版式 + 一组配色
    └── <包>/                        #   主题包，目录名即 id
        ├── index.ts                 #   包定义，同时是该包的默认配色（token 取值）
        ├── theme.scss               #   （可选）包默认配色的背景图 / webfont / 媒体查询
        ├── <page>/index.vue         #   基础版式（直接落在页面目录下，无 base/ 层）
        ├── <page>/<变体>/index.vue  #   变体：`import.meta.glob` **惰性**加载
        └── colors/<配色>/           #   该包下的其它配色（index.ts + theme.scss + assets/）
```

- `<包>`、`<配色>`、`<变体>` 都只允许 `[a-z0-9-]`（`<变体>` 会成为 `data-mauth-view` 的属性值）。
...
| `themes/<包>/colors/<配色>/` | 该包下的**配色**（`ocean` / `sky` / …） | **目录名就是 id** |
...
| 包内 `meta.id` 与目录名不一致 | **以目录名为准**（`meta: { ...pkg.meta, id: pkg }`） |
...
view/app/<page>/index.vue                 业务容器：状态 / 校验 / 请求 / 路由 / 浮层 / 倒计时
        ...
        ▼
themes/default/<page>/index.vue           基础版式（默认，内置包静态引入）
themes/<包>/<page>/<变体>/index.vue       变体（懒加载）
```
...
⚠️ **前两档都在当前主题包内查找**（版式跟随主题包）：某个包没登记这套版式时，回退的是**该包自己的**
基础版式，不跨包借别人的版式。
...
- **内置包 `base` 静态引入**（`import BaseXxxView from '@/theme/themes/default/<page>/index.vue'`）→ 默认路径**零额外请求**。
...
3. **基础版式**：建 `themes/default/<page>/index.vue`，把容器原来的模板**忠实搬运**过去（不改外观），
...
1. 建目录 `src/theme/themes/<包>/colors/<配色>/`（`<配色>` 只允许 `[a-z0-9-]`）。
...
1. 建目录 `themes/<包>/<page>/<变体>/`（基础版式则是直接改 `<page>/index.vue`）。
...
参考实现照抄 `themes/default/register/compact/`。
...
| 配色 id | `theme/index.ts` 注册表 | 同上；未登记回退 `default` |
...
2. **目录口径关卡**：`.tmp-probe/verify-theme-dirs.mjs`（61 项静态断言，纯文件系统、**不需要浏览器**）。
   守的就是本节「主题包 = 一个目录」那张表：目录名合法性、必备文件、`theme.scss` 选择器必须用目录名、
   配色目录叫 `colors/`、版式去 `base/` 层、契约收在 `theme/views/` 下，
   外加**实现 ↔ 文档口径一致性**（glob 仍只扫两层、仍由目录名覆盖 `meta.id`）。
...
| 坑 | 症状 | 处置 |
| --- | --- | --- |
| 加了配色/主题目录但"没生效" | 主题不出现，**且没有任何报错** | 注册表对坏目录是**静默跳过**：逐一核 ① 目录名是否 `[a-z0-9-]` ② 有没有 `index.ts` ③ 有没有 `export default` ④ 包里有没有 `meta` |
...
| 页面 | 容器 | 版式目录 | 已登记版式 | 预取 |
| --- | --- | --- | --- | --- |
| 注册 | `view/app/register/index.vue` | `themes/default/register/` | `base`、`compact` | ✅ |
| 登录 | `view/app/login/index.vue` | `themes/default/login/` | `base` | ✅ |
| 重置密码 | `view/app/forgot-password/index.vue` | `themes/default/forgot-password/` | `base` | ✅ |

主题包：`default`（版式 + 默认配色 + 配色 `ocean` / `sky`）。
配色：`ocean`（只改取值）、`sky`（改取值 + 改结构 + 横屏断点）。
