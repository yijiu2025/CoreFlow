...
`accountKey = HMAC-SHA256(uid, SESSION_SECRET)` 前 16 hex，形如 `k_904eedea7fa862e9`。

它是**唯一标识**，同时充当三个角色：

| 角色 | 说明 |
| --- | --- |
| HttpOnly 凭证 cookie 的名字 | 后端 `request.cookies[accountKey]` 直接读 refreshToken |
| 前端 localStorage 的 key | `{[accountKey]: {username, avatar}}`，同 uid → 同 HMAC → 自动去重 |
| 切账号时前端发送的值 | 后端无需解密，直接当 cookie 名读 rt |

**特性**：确定性（同 uid 永远相同，自动去重）、不可逆（非明文 uid）、随 `SESSION_SECRET` 变化（换密钥则所有 accountKey 失效 = 强制全部重新登录）。
...
| `k_<HMAC(uid)>` | ✅ | `/` | 某账号的 refreshToken（180d，记住我账号才有） | 后端 bind-session | 后端 switch-account |
| localStorage `saved_accounts` | ❌（前端） | — | `{[accountKey]: {username, avatar}}`，仅展示 | 前端 addSavedAccount | 前端 LoginModal |

**关键点**：真正的免切凭据是 HttpOnly cookie `k_<accountKey>`，JS 读不到（防 XSS 窃凭证）。localStorage 只存 accountKey + 展示信息，不含任何凭证。
...
[iframe 登录成功]
  ...
  ├─ sessionToken → POST /auth/v1/bind-session → 后端写 sid/sid_r/k_<accountKey> cookie，返回 {user, accountKey}
  └─ accountKey → authStore.addSavedAccount(user, accountKey) → 存 localStorage
...
用户点账号卡片 → onSwitchAccount(acct)
  authStore.switchAccount(acct.accountKey)
    → POST /auth/v1/switch-account {accountKey}
    → 后端 request.cookies[accountKey] 读 refreshToken
...
点账号卡片 × → onRevoke(acct)
  authStore.revokeSavedAccount(acct.accountKey)
    → POST /auth/v1/saved-accounts/revoke {accountKey}
    → 后端读 cookie[accountKey] 的 rt → revokeRememberMe + 清该 cookie + 清 sid_r
...
| 端点 | 方法 | 入参 | 返回 |
| --- | --- | --- | --- |
| `/auth/v1/bind-session` | POST | `{session_token}` | `{user, accountKey}` （accountKey 仅记住我时非空） |
| `/auth/v1/switch-account` | POST | `{accountKey}` | `{action:'switched', user}` 或 `{action:'need_password'}` |
| `/auth/v1/saved-accounts/revoke` | POST | `{accountKey}` | `{}` |
...
| 风险 | 防护 |
| --- | --- |
| XSS 窃取长期凭证 | refreshToken 全程在 HttpOnly cookie（`k_<accountKey>`），JS 读不到；localStorage 不存任何凭证 |
| 明文 uid 暴露 | accountKey = HMAC(uid)，不可逆；无明文 uid 下发（旧的 `accounts` 明文 cookie 已删除） |
...
| 重复登录产生多条 | accountKey 确定性（同 uid → 同 HMAC），localStorage 以 accountKey 为 key 天然去重 |
| 凭证泄露轮换 | 换 `SESSION_SECRET` → 所有 accountKey 失效 → 全部强制重新登录 |
