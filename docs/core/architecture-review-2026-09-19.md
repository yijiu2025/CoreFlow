...
**结论**：该文档是一项**设计方案**，但**没有标注实现状态**。文档也**没有出现在任何导航索引里**（`docs/deployment/cicd.md`、`docker.md` 同）。

**处置**：在文档顶部加实现状态徽章（`📐 设计稿 · 未实现` / `✅ 已实现`）；
或把设计稿移到 `docs/plans/`（该目录已有同类内容）。
...
**处置**：要么补齐制品，要么把路线图的完成标记改回未完成。**建议补齐**（见 §4 阶段 1），
因为多实例部署的前置条件正是一个可复现的构建产物。
...
| 0.2 | 角色批量写入 | `app/user/dao/user.js:196` | 改 `bulkCreate`；注册流程回归测试全绿 |
| 0.3 | 补连接超时 | `src/app.js:172` | ✅ **已完成 2026-09-19**：1 项生效 + 2 项实测无效（见 C-3），核心是 WS `preClose`；含反例对照 |
| 0.4 | 修依赖声明 | `package.json` | `wb-logkit` 改为与本地版本一致 |
| 0.5 | 清理死文件 | 根 `data/` | 确认无引用后删除，或加废弃说明 |
| 0.6 | 文档加实现状态 | `docs/deployment/multi-server.md`、`docs/roadmap/index.md` | 加 `📐 设计稿 · 未实现` 徽章；修正 Docker 完成标记 |
...
| # | 动作 | 说明 |
| --- | --- | --- |
| 1.1 | **健康探针分级** | `/health/live`（永远 200）+ `/health/ready`（关键依赖不可用 → 503）。这是后续一切扩容与自愈的**判据基础**，优先级最高 |
| 1.2 | **启动失败分级** | loader 区分关键/可选加载器；关键失败 fail-fast。可选失败在 `/health/ready` 暴露明细 |
...
| P1 | 启动失败分级（1.2） | 🟡 C-1 | 小 | 消除"服务在跑但功能缺失" | |
| P1 | 部署制品补齐（1.4） | 🟡 B-2 | 中 | 扩容的前置条件 | |
| P1 | 文档实现状态标注（0.6 / B-1 / D-4） | 🟡 B | 极小 | 防止基于错误前提做决策；含修正 3 处 `data/guard_config.json` 误导描述 | |
