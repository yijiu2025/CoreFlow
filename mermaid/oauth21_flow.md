...
- **登录入口 (`StandardLogin.vue`/`MiniLogin.vue`)**：
  - _功能_：提供标准的用户名密码登录，同时支持基于 H5 签名的防爬虫防刷机制。可由第三方客户端发起授权时唤起。
- **同意授权页 (`Authorize.vue`)**：
  - _功能_：这是 OAuth 2.1 中的核心用户交互页面。展示第三方应用所申请的 Scope 范围权限（如 openid, profile, email），并在用户确认后，向后端请求授权码（Authorization Code），最后重定向回客户端的回调地址。
